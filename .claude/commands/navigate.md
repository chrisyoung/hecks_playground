...
```bash
rust/target/release/hecks_playground-life lexicon hecks_playground_conception 2>&1 | grep "→" | awk -F'→' '{print $2}' | awk -F'::' '{print $1}' | sed 's/^ *//' | sort -u
```
...
```bash
rust/target/release/hecks_playground-life lexicon hecks_playground_conception 2>&1 | grep "→.*DomainName::" | grep -v " then "
```
...
```bash
rust/target/release/hecks_playground-life heki append hecks_playground_conception/information/<Domain>.heki domain=<Domain> aggregate=<Aggregate> command=<Command> param1=value1 param2=value2
```
