...
## Persistence is aggregate-only (sprint-14, 2026-05-31)

- **Persistence is an adapter BELOW the aggregate ; the CLI exposes the DOMAIN, never the STORAGE.** Read paths route through `storehouse query <root> Domain::Aggregate.snake_case k=v` ; write paths route through `storehouse <root> Domain::Aggregate.Command k=v`. The aggregate IS the conceptual layer — readers and writers BOTH enter through it so the same query name, payload shape, and authorisation gate apply to every caller.
- **The `storehouse heki` read family is a leak, not a contract.** `heki read|latest|snapshot|retain|get|list|count|ids|next-ref|latest-field|values|mark|seconds-since` all take a `.heki` file path and project storage rows directly, bypassing the owning aggregate's queries. They survive as a development-time inspector ; never reach for them from production code or new tooling. The write side (`heki append|upsert|delete`) was already mechanically retired in sprint-14 (`make-heki-direct-write-impossible`) — the read side stays for now, on a leash.
- **Sniff test for new subcommands** — if the verb's name matches a persistence technology (heki / sqlite / kv / parquet / fs / json / yaml), it is an adapter, not a verb. Push it below the aggregate and expose the equivalent aggregate Query instead. Card `bluebook-derived-cli-surface` tracks growing the CLI from bluebook IR so no future verb gets hand-added in Rust.
- See `~/Projects/miette/discipline/anti_patterns.bluebook` — `PersistenceIsAggregateOnly` instance.
