...
- **Stochastic & periodic given primitives** (i229) — `given { rand_below(N) == 0 }` is a 1/N stochastic gate (uniform random in `[0, N)`); `given { <expr>.modulo(N) == 0 }` is an every-Nth periodic gate (resolves the receiver as int, returns `% N`). Both lift legacy shell-side cadence math (`if [ $((RANDOM % N)) -eq 0 ]`, `if (tick % N).zero?`) into bluebook IR ; `HECKS_RAND_SEED` pins the RNG for tests ; non-positive N short-circuits to 0 (predicate fires every call) so daemons degrade gracefully on typos. Ruby parity mirror in `ruby/hecks/behaviors/interpreter.rb`. See `docs/usage/given_predicate_primitives.md`.
...
- **No-primitive-envy hygiene** — every primitive wrapped in a typed value-object (`Name`, `Timestamp`, `FilePath`, `ComplaintText`, `EditCount`, `ViolationCount`, etc.) so the domain's language stays load-bearing; same sweep also wrapped primitives in `Immunity`, `Antibody`, `dispatch_audit`, `mindstream`, plus `cli/{cli,console,status,statusline,banner}` and `integrations/{cloudflare_deploy,web_application_creation}` on 2026-05-12
...
- **Tools bluebook** (`hecks_conception/aggregates/framework/tools/tools.bluebook`) — lifts agent tool invocations into first-class dispatched commands. Restructured 2026-05-12 from a flat single-`Tools` aggregate into **five category aggregates** : `ShellTool` (`Bash`), `FileTool` (`Read` / `Edit` / `Update`), `SearchTool` (`Grep` / `Glob`), `WebTool` (`WebFetch` / `WebSearch`), `Cascade` (`RecordResult`). Each aggregate `identified_by :id` ; the harness mints a stable ULID before dispatch ; a Cascade record shares the originating tool record's id so the outcome is joinable by id across aggregates. See i551
- **`:claude_tool` adapter family** (`framework/adapter_families/claude_tool.hecksagon`) — sibling to `:sms`, `:tts`, `:llm`, `:compute`. Each instance binds one tool-category command to one tool kind (`:bash`, `:edit`, `:read`, `:write`, `:grep`, `:glob`) and names a `result_into` follow-on dispatch (now `"Cascade.RecordResult"`). The kernel resolves the matching adapter at dispatch time, fires the `invoke_claude_tool` behavior, captures the result, and routes it back as a follow-on dispatch
...
- **`claude_tool_dispatcher.rs` kernel hook** (`rust/src/runtime/claude_tool_dispatcher.rs`) — 346 lines, six native primitives. End-to-end `ShellTool.Bash` dispatch actually runs the shell via `std::process::Command`; `FileTool.Edit / Read / Update` go through `std::fs`; `SearchTool.Grep / Glob` walk the filesystem. The contract stays unchanged if/when the runtime upgrades to call THROUGH a Claude API rather than execute natively. See i556
...
- **Category split rationale** — sibling commands of unrelated kinds no longer share one record. Each category aggregate's state focuses on what that kind of invocation actually carries (ShellTool: shell_command ; FileTool: file_path ; SearchTool: pattern + search_path ; WebTool: url / query) ; the Cascade aggregate separates the outcome-chain concern from the invocation concern. Per the locked 2026-05-12 conventions, every value object is declared INSIDE the aggregate that uses it ; duplication across aggregates (Id, Description, FilePath) is fine
- **Cross-repo `Tools.*` invocation** (2026-05-12) — `storehouse ShellTool.Bash …` works from any cwd once `~/bin/storehouse` is symlinked to `~/Projects/hecks/rust/target/release/storehouse`. The CLI recognises an `Aggregate.Command` first positional, resolves the conception via `HECKS_CONCEPTION_DIR` env var (falling back to the canonical `~/Projects/hecks/hecks_conception`), and dispatches without requiring the explicit conception path. The positional form (`storehouse <conception> ShellTool.Bash …`) still works and takes precedence. See `docs/usage/tools.md` "Setup" section
- **EmailTool `:mcp` adapter bindings** (2026-05-14, i608) — four `adapter :mcp` rows in `framework/tools/tools.hecksagon` bind `EmailTool.SearchThreads`, `EmailTool.GetThread`, `EmailTool.CreateDraft`, `EmailTool.ListDrafts` to the Claude harness's Gmail MCP surface (`mcp__claude_ai_Gmail__search_threads` / `_get_thread` / `_create_draft` / `_list_drafts`). Args mapping : `query→q`, `thread_id→thread_id`, `to→to`/`subject→subject`/`body→body_markdown`, ListDrafts no args. Each binding chains `result_into: "Cascade.RecordResult"` so the Gmail response cascades onto a Cascade record sharing the originating invocation id. The bindings ARE the contract today ; end-to-end execution lights up when i594 (bluebook-driven kernel-hook registration) and the `:gmail` server registration in the MCP dispatcher's `resolve_server_spawn` close. See `docs/usage/email-tool.md`

### MCP Universal Dispatcher (2026-05-13)
- **Hand-registered sugar retired** — the 9 `storehouse__*` per-command MCP wrappers (`_bash`, `_read`, `_edit`, `_update`, `_grep`, `_glob`, `_web_fetch`, `_web_search`, `_record_result`) and the `dispatch.mjs` bus-tool runner are deleted. The MCP layer no longer duplicates the bluebook contract per-command — the bluebook IS the contract
- **10 MCP tools, down from 16** : `storehouse__dispatch` (universal door — any bluebook command on any aggregates root), `storehouse__query` (read-only verb dispatch, snake_case tail enforced), `storehouse__state` (aggregate state by id), three IR-discovery zooms (`storehouse__catalog` / `_describe_aggregate` / `_list_aggregates`), plus the developer workflow tools (`_validate`, `_macrophage_check`, `_behaviors`, `_conceive_behaviors`)
- **New `storehouse` subcommands** — `describe <bluebook> <aggregate>` emits canonical-IR JSON for one aggregate (via the existing `dump::dump_aggregate`) ; `query <root> <verb> k=v ...` dispatches a snake_case query verb (enforces query shape — PascalCase rejected with a pointer to `dispatch`) ; `state <root> <aggregate> <id>` reads `Runtime::find` and emits `{ ok, aggregate, id, state }` (missing record returns `ok:false` with `state:null`, not exit 1) ; the existing `dump` subcommand is the catalog source
- **FQN required at runtime** — `Domain::Aggregate.Command` for commands (PascalCase), `Domain::Aggregate.snake_case` for queries. Short-form `Tools.Bash` is rejected with a pointer to the qualified form. The `Tools::Tools.Bash` → `Tools::ShellTool.Bash` 5-aggregate split (2026-05-12) is the canonical address book
- **Dynamic discovery, not static registration** — the LLM constructs the verb string from the IR catalog rather than seeing per-command typed tools. `storehouse__catalog` returns full IR JSON for the bluebook ; `storehouse__describe_aggregate` zooms to one aggregate ; the verb string flows through `storehouse__dispatch` at call time. No per-bluebook-command MCP-tool registration ever happens
- **End-to-end smoke** — `node test/cli_smoke.mjs` passes 46/46 (10 tools registered, 11 retired tools asserted gone, every wrapper exercised against the World bluebook + a synthetic ShellTool dispatch + state read-back)
- **Renames** : `storehouse__dispatch_command` → `storehouse__dispatch` ; `storehouse__list` retired in favour of `storehouse__catalog` + `storehouse__describe_aggregate`. See `docs/usage/storehouse-mcp.md`
