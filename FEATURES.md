...
- **Cross-repo `Tools.*` invocation** (2026-05-12) — `storehouse Tools.Bash …` works from any cwd once `~/bin/storehouse` is symlinked to `~/Projects/hecks/rust/target/release/storehouse`. The CLI recognises an `Aggregate.Command` first positional, resolves the conception via `HECKS_CONCEPTION_DIR` env var (falling back to the canonical `~/Projects/hecks/hecks_conception`), and dispatches without requiring the explicit conception path. The positional form (`storehouse <conception> Tools.Bash …`) still works and takes precedence. See `docs/usage/tools.md` "Setup" section
