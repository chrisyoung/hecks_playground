...
- **Anything in `rust/src/` under hecks_playground** — that's kernel-floor, Miette's territory.
...
3. **Bluebook-first applies inside hecks_playground-managed repos** : if you're modifying `.rb` / `.rs` / `.sh` / `.js` inside one, the antibody may fire. Stop and report — don't pre-write exemption markers.
