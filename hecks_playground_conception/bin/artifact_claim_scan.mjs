...
//
// cwd : runtime process cwd (hecks_playground_conception/), same as the verify leaf.
...
const REPO_ROOT = path.resolve(HECKS_PLAYGROUND_CONCEPTION, "..");
const STOREHOUSE = path.join(REPO_ROOT, "rust", "target", "release", "storehouse");
const SCAN_ROOT = path.join(HECKS_PLAYGROUND_CONCEPTION, process.env.ARTIFACT_SCAN_ROOT || "aggregates");
...
function recordClaim(claimId, source, claimedPath, context) {
  return sh([HECKS_PLAYGROUND_CONCEPTION, "Discipline::ArtifactClaim.RecordClaim",
...
function retract(claimId) {
  return sh([HECKS_PLAYGROUND_CONCEPTION, "Discipline::ArtifactClaim.Retract",
    `artifact_claim=${claimId}`]).status === 0;
}

// Claims already on record for one source (for reconciliation).
function claimsForSource(source) {
  const r = sh(["query", HECKS_PLAYGROUND_CONCEPTION, "Discipline::ArtifactClaim.for_source",
    `source=${source}`, "--json"]);
...
function main() {
  ...
  for (const file of files) {
    const source = path.relative(HECKS_PLAYGROUND_CONCEPTION, file);
