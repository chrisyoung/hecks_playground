...
// artifact lives — bin/ + aggregates/ resolve under hecks_playground_conception/,
...
//
// cwd : the runtime process cwd (hecks_playground_conception/), same convention as
...
function candidates(claimed) {
  ...
  return [path.join(HECKS_PLAYGROUND_CONCEPTION, claimed), path.join(REPO_ROOT, claimed)];
}
...
function unverifiedClaims() {
  const r = spawnSync(STOREHOUSE,
    ["query", HECKS_PLAYGROUND_CONCEPTION, "Discipline::ArtifactClaim.unverified", "--json"],
    { encoding: "utf8" });
...
function mark(command, claimId) {
  const r = spawnSync(STOREHOUSE,
    [HECKS_PLAYGROUND_CONCEPTION, `Discipline::ArtifactClaim.${command}`,
