...
//      against the .overmind.sock in hecks_playground_conception/. If the
//      socket is missing OR ps returns non-zero, the supervisor is
//      dead — `overmind start -D` from hecks_playground_conception/ brings
...
// runs from hecks_playground_conception/, so relative paths resolve identically
...
const HECKS_PLAYGROUND_CONCEPTION = path.resolve(HERE, "..");
const STOREHOUSE = path.join(HECKS_PLAYGROUND_CONCEPTION, "..", "rust", "target", "release", "storehouse");
const OVERMIND_SOCK = path.join(HECKS_PLAYGROUND_CONCEPTION, ".overmind.sock");
...
function dispatchMarkAlive(processName) {
  ...
  const r = run(STOREHOUSE, [
    path.join(HECKS_PLAYGROUND_CONCEPTION, "aggregates"),
...
function main() {
  ...
  if (socketAlive) {
    const ps = run(overmind, ["ps"], { cwd: HECKS_PLAYGROUND_CONCEPTION });
  ...
  if (!supervisorAlive) {
    ...
    const start = run(overmind, ["start", "-D"], { cwd: HECKS_PLAYGROUND_CONCEPTION });
    if (!start.ok) {
  ...
  for (const name of dead) {
    ...
    const r = run(overmind, ["restart", name], { cwd: HECKS_PLAYGROUND_CONCEPTION });
    if (!r.ok) {
