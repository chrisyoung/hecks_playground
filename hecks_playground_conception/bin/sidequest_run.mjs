...
if (process.env.STOREHOUSE_TRIGGER_EVENT) {
  ...
  const candidates = [
    // config-driven first (HECKS_PLAYGROUND_BEING_STATE) ; literals are TRANSITIONAL.
    ...(process.env.HECKS_PLAYGROUND_BEING_STATE ? [`${process.env.HECKS_PLAYGROUND_BEING_STATE}/information/sidequest/sidequest.heki`] : []),
    `${HOME}/Projects/miette-state/information/sidequest/sidequest.heki`,
    `${HOME}/Projects/hecks_playground/hecks_playground_conception/information/sidequest/sidequest.heki`,
  ];
