...
**Status:** IMPLEMENTED 2026-06-19 (829e67b83) — `HECKS_PLAYGROUND_INFO` is removed and the
...
- `dir "~/.heki"` everywhere.
- **There is NO environment variable.** `HECKS_PLAYGROUND_INFO` precedence is dropped ;
...
2. **Full pizzas-ification** of adapters : migrate every `adapter :heki`
   ...
   (~22 hecksagons in hecks_playground_conception alone, plus miette + client repos.)
...
- `rust/src/heki.rs:656 resolve_info_dir()` reads `HECKS_PLAYGROUND_INFO` first ; tests in
...
3. Drop `HECKS_PLAYGROUND_INFO` precedence in `resolve_info_dir` (becomes last-resort
   default only, or removed) ; invert the `resolve_tests`.
4. **VERIFY against pizzas** : `rm -rf ~/.heki/<realm>/pizzas` ; dispatch
   `Pizzas::Pizza.CreatePizza ...` with `HECKS_PLAYGROUND_INFO` UNSET ; confirm the store
...
For every domain with `adapter :heki` across `~/Projects` (hecks_playground_conception
...
- Realm value per repo : hecks_playground_conception → ? (“miette”? “hecks_playground”?), clients →
...
4. **Still wanted (fan-out, task #6) :** hecksagons WRITTEN + adapters HOOKED
   ...
   documenting `:default`.
5. **HECKS_PLAYGROUND_INFO retires (task #7, SEPARATE arc) :** making folder-chain the
   live default moves my OWN organs off `miette-state/information`. Keep
   folder-chain HECKS_PLAYGROUND_INFO-gated (env still wins while set) until the
...
- pizzas.world switched from `realm "HecksPlayground"` override to `dir :default` ;
  e2e (HECKS_PLAYGROUND_INFO unset) lands at ~/.heki/hecks_playground/examples/pizzas/pizza.heki.
...
1. **Per-aggregate source-path threading** — `derive_default_chain` works for
   ...
   `hecks_playground_conception`, flattening organs to ~/.heki/hecks_playground/<domain>). Fix :
...
- **NO hand-coded realms for now.** Every domain uses `:default`
  ...
  already switched off its `realm "HecksPlayground"` override to `dir :default`.
- **Everything under `~/.heki`, OUTSIDE `~/Projects`** — so live state needs no
  ...
  ~/.heki."
- **The precise pass is LATER.** First pull embryonaut features OUT of hecks_playground so
...
fresh-head-this-week.** My organs stay on HECKS_PLAYGROUND_INFO / miette-state until the
...
copy + HECKS_PLAYGROUND_INFO retirement + daemon restart.
