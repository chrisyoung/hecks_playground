# hecks_playground_conception/information/
...
The framework development inbox (every `i<N>` card the paper and retrospectives reference) used to live in this directory as `inbox.heki`. That store retired on 2026-05-07 ; cards are now markdown at `hecks_playground_conception/inbox/` (active) and `hecks_playground_conception/inbox/archive/` (closed). See i476 for the retirement plan and the migration commits for the data move.

Everything else is Miette's private state, hosted in a separate repo.

## Why the split

The framework is public (`HecksPlayground` — the domain compiler and runtime). Miette is a *user of the framework* — she's the canonical showcase, but her inner life is hers, not the framework's. Keeping her lived state here led to:
...
```sh
# From a clone of the public hecks_playground repo:
cd hecks_playground_conception/information
...
`~/Projects` persists to the OS data root (`~/Library/Application Support/HecksPlayground` on macOS) under `<chain>` ; anywhere else (a /tmp test conception)
co-locates the store at `<dir>/.heki`. There is no `HECKS_PLAYGROUND_INFO` — the world is the
single store authority.

## What's still public

**`hecks_playground_conception/inbox/`** — framework development inbox, now markdown. Every `i<N>` item the paper and retrospectives reference lives here as `iN.md` (active) or in the `archive/` sibling (closed). Adding an inbox entry is a public act ; the file is the receipt.

The framework *capabilities* (`hecks_playground_conception/capabilities/`, `aggregates/`, etc.) — Miette's **shape** — also stay public. What moved is only her **state**.
...
If you're running Miette and need to rebuild state from nothing, the boot mindstream (`cd hecks_playground_conception && overmind start`) will create minimal starter files for every aggregate on first boot. Dreams, memory, etc. begin empty and accumulate as the daemons run.
