...
seed() {
  ...
  "$HECKS_PLAYGROUND" heki append "$d/mood.heki"          current_state="$2"               >/dev/null 2>&1
  "$HECKS_PLAYGROUND" heki append "$d/consciousness.heki" state="$3" sleep_stage="$4" is_lucid="$5" >/dev/null 2>&1
  "$HECKS_PLAYGROUND" heki append "$d/heartbeat.heki"     fatigue_state="$6" pulses_since_sleep="$7" >/dev/null 2>&1
  "$HECKS_PLAYGROUND" heki append "$d/tick.heki"          cycle="$8"                        >/dev/null 2>&1
  [ -n "$9" ] && "$HECKS_PLAYGROUND" heki append "$d/lucid_dream.heki" latest_narrative="$9" >/dev/null 2>&1 || true
  echo "$d"
}
