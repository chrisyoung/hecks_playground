...
if [ -n "${HECKS_PLAYGROUND_BIN:-}" ]; then
  HECKS_PLAYGROUND="$HECKS_PLAYGROUND_BIN"
elif [ -x "$REPO_ROOT/rust/target/release/storehouse" ]; then
  HECKS_PLAYGROUND="$REPO_ROOT/rust/target/release/storehouse"
elif [ -x "$MAIN_REPO/rust/target/release/storehouse" ]; then
  HECKS_PLAYGROUND="$MAIN_REPO/rust/target/release/storehouse"
elif [ -x "$MAIN_REPO/rust/target/release/storehouse" ]; then
  HECKS_PLAYGROUND="$MAIN_REPO/rust/target/release/storehouse"
else
...
mkdir -p "$TMP/hecks_playground_conception/information" "$TMP/hecks_playground_conception/aggregates"
find "$CONCEPT_DIR/aggregates" -name "*.bluebook" -exec ln -sf {} "$TMP/hecks_playground_conception/aggregates/" \;

MIETTE_BODY="$MAIN_REPO/../miette/body"
if [ -d "$MIETTE_BODY" ]; then
  find "$MIETTE_BODY" -name "*.bluebook" -exec ln -sf {} "$TMP/hecks_playground_conception/aggregates/" \;
fi

AGG="$TMP/hecks_playground_conception/aggregates"
...
# — automatically isolated from the live OS data root, no HECKS_PLAYGROUND_INFO, no literal path.
cat > "$TMP/hecks_playground_conception/body_cycles_smoke.world" <<EOF
...
fail() { echo "FAIL — $1"; "$HECKS_PLAYGROUND" heki read "$STORE" 2>/dev/null | sed 's/^/    /'; exit 1; }

cd "$TMP/hecks_playground_conception"

# ── 1. Ultradian fast-forward (i106 multi-command rotation) ──────
"$HECKS_PLAYGROUND" loop "$AGG" Body::Ultradian.EnterPeak,Body::Ultradian.EnterTrough --every 1s >/dev/null 2>&1 &
PID=$!
sleep 2.5
kill "$PID" 2>/dev/null
wait "$PID" 2>/dev/null
...
cycle_count=$("$HECKS_PLAYGROUND" heki latest-field "$STORE" cycle_count 2>/dev/null || echo 0)
phase=$("$HECKS_PLAYGROUND" heki latest-field "$STORE" phase 2>/dev/null || echo "")
...
"$HECKS_PLAYGROUND" "$AGG" Mind::Consciousness.EnterDaydream >/dev/null 2>&1 \
  || fail "EnterDaydream dispatch failed"
"$HECKS_PLAYGROUND" "$AGG" Mind::Consciousness.EnterSleep >/dev/null 2>&1 \
  || fail "EnterSleep dispatch failed"

"$HECKS_PLAYGROUND" loop "$AGG" Body::Heart.Beat \
...
heart_count=$("$HECKS_PLAYGROUND" heki latest-field "$STORE" beat_count 2>/dev/null || echo 0)
...
"$HECKS_PLAYGROUND" "$AGG" Mind::Consciousness.WakeUp >/dev/null 2>&1 \
...
after_gate=$("$HECKS_PLAYGROUND" heki latest-field "$STORE" beat_count 2>/dev/null || echo 0)
