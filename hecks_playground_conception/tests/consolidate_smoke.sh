...
for i in 1 2 3; do
  "$HECKS_PLAYGROUND" "$TMP/aggregates" Body::Signal.FireSignal \
...
for t in doomed_a doomed_b; do
  "$HECKS_PLAYGROUND" "$TMP/aggregates" Body::Synapse.CreateSynapse \
    from="$t" to="$t" strength=0.05 >/dev/null 2>&1 \
    || fail "Body::Synapse.CreateSynapse $t dispatch failed"
done

signals_before=$("$HECKS_PLAYGROUND" heki read "$STORE/signal/signal.heki" 2>/dev/null \
  | python3 -c 'import sys,json; d=json.load(sys.stdin); print(len(d))' 2>/dev/null || echo 0)
synapses_before=$("$HECKS_PLAYGROUND" heki read "$STORE/synapse/synapse.heki" 2>/dev/null \
  | python3 -c 'import sys,json; d=json.load(sys.stdin); print(len(d))' 2>/dev/null || echo 0)
...
"$HECKS_PLAYGROUND" "$TMP/aggregates" Mind::Consciousness.EnterDaydream >/dev/null 2>&1 \
  || fail "EnterDaydream dispatch failed"
"$HECKS_PLAYGROUND" "$TMP/aggregates" Mind::Consciousness.EnterSleep >/dev/null 2>&1 \
  || fail "EnterSleep dispatch failed (SleepEntered policy chain)"
"$HECKS_PLAYGROUND" "$TMP/aggregates" Mind::Consciousness.ElapsePhase >/dev/null 2>&1 \
  || fail "ElapsePhase dispatch failed (PhaseElapsed → Consolidation sweep)"

store_rows=$("$HECKS_PLAYGROUND" heki read "$STORE/store/store.heki" 2>/dev/null \
  | python3 -c 'import sys,json; d=json.load(sys.stdin); print(len(d))' 2>/dev/null || echo 0)
remains_rows=$("$HECKS_PLAYGROUND" heki read "$STORE/remains/remains.heki" 2>/dev/null \
  | python3 -c 'import sys,json; d=json.load(sys.stdin); print(len(d))' 2>/dev/null || echo 0)
