...
# / HECKS_PLAYGROUND_LLM_PROVIDER=test wiring for deterministic dream-content
...
BODY_DIR="${HECKS_PLAYGROUND_BODY_DIR:-}"
...
Hecks.world "DreamContentSmoke" do
  ...
  # store at $TMP/aggregates/.heki — isolated from the live OS data root, no HECKS_PLAYGROUND_INFO.
  heki do
    dir :default
  end
end
...
"$HECKS_PLAYGROUND" heki upsert "$STORE/consciousness/consciousness.heki" \
...
count_records() {
  [ ! -f "$1" ] && { echo 0; return; }
  "$HECKS_PLAYGROUND" heki count "$1" 2>/dev/null || echo 0
}

field_value() {
  [ ! -f "$1" ] && { echo ""; return; }
  "$HECKS_PLAYGROUND" heki latest-field "$1" "$2" 2>/dev/null || echo ""
}
...
# routing edge cases.
HECKS_PLAYGROUND_LLM_PROVIDER=test \
HECKS_PLAYGROUND_AGG="$TMP/aggregates" \
HECKS_PLAYGROUND_BIN="$HECKS_PLAYGROUND" \
"$HECKS_PLAYGROUND" run-loop "$TMP/aggregates" \
  --every 500ms \
...
"$HECKS_PLAYGROUND" "$TMP/aggregates" 'BodyDream::Dream.ProduceImage' \
  name=dream sleep_cycle=1 >/dev/null 2>&1 || true
DREAM_OUTBOX=$("$HECKS_PLAYGROUND" query "$TMP/aggregates" \
...
if [ -n "$DREAM_OUTBOX" ] && [ "$DREAM_OUTBOX" != "null" ]; then
  DREAM_VERDICT=$(printf '%s' "$DREAM_OUTBOX" | HECKS_PLAYGROUND_LLM_PROVIDER=test "$BODY_DIR/dream/dream-image-handler" 2>/dev/null)
  "$HECKS_PLAYGROUND" "$TMP/aggregates" 'BodyDream::Dream.RecordImage' \
...
reading=$("$HECKS_PLAYGROUND" heki list "$DREAM_HEKI" --format json 2>/dev/null | \
  jq -r '[.[].reading] | map(select(. != "" and . != null))[0] // ""' 2>/dev/null)
if [ -z "$reading" ] || [ "$reading" = "null" ]; then
  echo "----- run-loop output -----"
  cat "$RUN_LOG"
  echo "----- dream.heki -----"
  "$HECKS_PLAYGROUND" heki list "$DREAM_HEKI" --format json 2>/dev/null || echo "(no dream.heki yet)"
...
text_en=$("$HECKS_PLAYGROUND" heki list "$DREAM_HEKI" --format json 2>/dev/null | \
