...
dream_rows=$("$HECKS_PLAYGROUND" heki read "$STORE/body_dream/dream.heki" 2>/dev/null \
...
"$HECKS_PLAYGROUND" "$TMP/aggregates" Mind::Consciousness.WakeUp >/dev/null 2>&1 \
...
# so we glob.
consciousness_state=$("$HECKS_PLAYGROUND" heki latest-field "$STORE/consciousness/consciousness.heki" state 2>/dev/null || echo "")
...
if [ -n "$di_paths" ]; then
  for p in $di_paths; do
    rows=$("$HECKS_PLAYGROUND" heki read "$p" 2>/dev/null \
      | python3 -c 'import sys,json; d=json.load(sys.stdin); print(len(d))' 2>/dev/null || echo 0)
