...
# under ~/Projects, so :default isolates from the live OS data root — no HECKS_PLAYGROUND_INFO.
...
(cd "$TMP" && HECKS_PLAYGROUND_AGG="$TMP/aggregates" \
  "$HECKS_PLAYGROUND" "$TMP/aggregates" Body::Pulse.Emit name=pulse >/dev/null 2>&1) \
...
field() {
  "$HECKS_PLAYGROUND" heki latest-field "$1" "$2" 2>/dev/null || true
}
