...
# Find the storehouse binary. Prefer HECKS_PLAYGROUND_BIN override; otherwise the
...
# this tmpdir) — no HECKS_PLAYGROUND_INFO ; the world is the single store authority.
"$HECKS_PLAYGROUND" run-loop "$TMP/bluebooks" \
  --every 1s \
