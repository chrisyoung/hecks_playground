...
hecks_playground="${STOREHOUSE:-}"
if [ -z "$hecks_playground" ]; then
  for cand in \
    ...
    if [ -x "$cand" ]; then hecks_playground="$cand"; break; fi
  done
fi
...
# so it isolates from the live OS data root). No HECKS_PLAYGROUND_INFO ; the world is authority.
...
"$hecks_playground" "$tmp/aggregates" Body::Tick.MindstreamTick >/dev/null 2>&1 || true
"$hecks_playground" "$tmp/aggregates" Mind::Consciousness.BecomeAttentive >/dev/null 2>&1 || true
"$hecks_playground" "$tmp/aggregates" Body::Heartbeat.Steady >/dev/null 2>&1 || true
"$hecks_playground" "$tmp/aggregates" Mind::Mood.Focus >/dev/null 2>&1 || true
