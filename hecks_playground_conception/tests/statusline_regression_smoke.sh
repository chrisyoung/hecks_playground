...
if [ -n "${HECKS_PLAYGROUND_BIN:-}" ]; then
  ...
  HECKS_PLAYGROUND="$REPO_ROOT/rust/target/debug/storehouse"
elif [ -x "$MAIN_REPO/rust/target/release/storehouse" ]; then
  HECKS_PLAYGROUND="$MAIN_REPO/rust/target/release/storehouse"
elif [ -x "$MAIN_REPO/rust/target/debug/storehouse" ]; then
  HECKS_PLAYGROUND="$MAIN_REPO/rust/target/debug/storehouse"
else
  echo "storehouse binary not found" >&2
  exit 1
fi
export STOREHOUSE="$HECKS_PLAYGROUND"
...
# from the live OS data root — no HECKS_PLAYGROUND_INFO. NOTE the render reads a DIFFERENT store :
# `storehouse statusline` resolves its store via resolve_info_dir(), which walks
# up to the REAL repo's hecks_playground_conception/aggregates world (the live OS data root),
...
seed_via_bus() {
  "$HECKS_PLAYGROUND" "$WORK/aggregates" Body::Tick.MindstreamTick >/dev/null 2>&1 || true
  "$HECKS_PLAYGROUND" "$WORK/aggregates" Mind::Consciousness.BecomeAttentive >/dev/null 2>&1 || true
  "$HECKS_PLAYGROUND" "$WORK/aggregates" Body::Heart.Beat >/dev/null 2>&1 || true
}
