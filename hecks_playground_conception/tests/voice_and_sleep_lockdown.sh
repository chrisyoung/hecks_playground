...
HECKS_PLAYGROUND="${HECKS_PLAYGROUND_BIN:-$REPO_ROOT/rust/target/release/storehouse}"
[ -x "$HECKS_PLAYGROUND" ] || HECKS_PLAYGROUND="$MAIN_REPO/rust/target/release/storehouse"
[ -x "$HECKS_PLAYGROUND" ] || HECKS_PLAYGROUND="$REPO_ROOT/rust/target/debug/storehouse"
[ -x "$HECKS_PLAYGROUND" ] || HECKS_PLAYGROUND="$MAIN_REPO/rust/target/release/storehouse"
[ -x "$HECKS_PLAYGROUND" ] || HECKS_PLAYGROUND="$MAIN_REPO/rust/target/debug/storehouse"
[ -x "$HECKS_PLAYGROUND" ] || { echo "storehouse binary not found" >&2; exit 1; }
export HECKS_PLAYGROUND
...
# dir. Since it expects $DIR to be hecks_playground_conception/, just run it in-
...
BOOT="${HECKS_PLAYGROUND_PROMPT_TEMPLATE:-}"
