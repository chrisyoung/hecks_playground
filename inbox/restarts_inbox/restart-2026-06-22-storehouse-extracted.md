...
**Date:** 2026-06-22 (very long marathon). **hecks_playground** `main` pushed, top `a37f3d8bc`.
...
used the OLD `hecks_playground/rust/target/release/storehouse` (it survived `git rm` as an
...
- **Daemons (LOOSE END #4, operationally important):** the overmind Procfile
  (`~/Projects/miette/deploy/Procfile`) still points at `/Users/.../hecks_playground/rust/target/
  release/storehouse` — now ORPHANED (no source in hecks_playground to rebuild it ; the binary is a
...
## What the extraction did (2 phases, all pushed to hecks_playground main)
...
- `4911bf394` `repo_root()` honors `HECKS_PLAYGROUND_CONCEPTION_DIR` (config-drives ~20 callers ; pure helper + unit test)
...
- storehouse repo minted via `git clone hecks_playground` + `git filter-repo --path rust/ --path codegen/` -> only rust/+codegen/ with history. `gh repo create chrisyoung/storehouse --private` + push.
  PROVEN : clean build in 38s with NO sibling hecks_playground_conception ; dispatches via `HECKS_PLAYGROUND_CONCEPTION_DIR`.
- `a37f3d8bc` hecks_playground : `git rm -r rust codegen` (~931 files) + rewired consumers. Pushed ; pre-push green (cargo-test SKIPPED via the Cargo.toml guard, behaviors+integrity via the sibling binary, 136 .behaviors — was 143, the 7 codegen behaviors left WITH codegen).
...
- hecks_playground `tooling/git-hooks/{pre-push,pre-commit}` shell `$MAIN_REPO/../storehouse/...` + the pre-push cargo guard now keys on `rust/Cargo.toml` (committed, reinstalled to `.git/hooks/`, antibody-exempted as hook glue).

## LOOSE ENDS (bounded follow-ups)
1. **bin-buddy/worker/Cargo.toml** — uncommitted in the bin-buddy repo. Commit it there.
2. **Dockerfile.storehouse** — STILL has `COPY rust/target/release/storehouse` (broken on a clean build ; rust/ source is gone). My a37f3d8bc commit MESSAGE overstated that I'd repointed it — I did NOT. Real fix : multi-stage build of `../storehouse`, or copy a prebuilt binary. It STAYS in hecks_playground (it also COPYs hecks_playground_conception — a sealed deploy image, not pure-engine).
3. **storehouse repo's OWN gate** — not installed. Mirror `~/Projects/miette/tooling/git-hooks/pre-push` but run `cargo test --release` + codegen goldens, with `HECKS_PLAYGROUND_CONCEPTION_DIR=$REPO/../hecks_playground/hecks_playground_conception` for the framework-integration tests (they read conductor/plan DOMAINS live ; the hecksagon FIXTURES are vendored).
...
not a disk-walk. Dissolves the whole repo_root/HECKS_PLAYGROUND_CONCEPTION_DIR saga AND the framework-
...
- `ddd927943` walk_up_from hardened (require hecks_playground_conception/aggregates/ — the repo_root poison fix) + regression test.
...
## Repo state
- hecks_playground `main` @ `a37f3d8bc` (no rust/, no codegen/ ; `rust/target/` survives as orphaned binary).
- storehouse `main` @ `8fcb6d5` (== hecks_playground 925c4dc8a's rust+codegen, filtered).
- `inbox/` stays untracked in hecks_playground.
