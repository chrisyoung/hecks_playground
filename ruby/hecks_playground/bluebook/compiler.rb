...
require_relative "in_memory_loader"
  # HecksPlayground::BluebookCompiler
  #
  ...
  # respective target gems and self-register via HecksPlayground.register_target when
  # required. See hecks_playground_targets/ for implementations.
  #
  #   HecksPlayground.build(domain, version: "2026.03.23.1")
  #   HecksPlayground.load_domain(domain)
  #
...
module HecksPlayground
  module BluebookCompiler
    ...
    #
    # @param domain [HecksPlayground::BluebookModel::Domain] the domain to compile
    ...
    # @raise [HecksPlayground::ValidationError] if domain validation fails
    def build(domain, version: "0.1.0", output_dir: ".")
      valid, errors = validate(domain)
      unless valid
        raise HecksPlayground::ValidationError.for_domain(errors)
      end

      ValidationRules::Naming::ReservedNames.reserved_attr_warnings(domain).each do |w|
        warn "[HecksPlayground] Warning: #{w}"
      end
    ...
    #
    # @param domain [HecksPlayground::BluebookModel::Domain] the domain to load
    ...
    #   way HecksPlayground.boot does for directory-based projects.
    # @return [Module] the loaded domain module constant (e.g., PizzasDomain)
    # @raise [HecksPlayground::ValidationError] if validation fails and skip_validation is false
    # @raise [HecksPlayground::BluebookLoadError] if generated code has syntax or naming errors
    def load_bluebook(domain, force: false, skip_validation: false, source_dir: nil)
      ...
      HecksPlayground::Utils.remove_constant(mod.to_sym)
      begin
        HecksPlayground::InMemoryLoader.load(domain, mod)
      rescue SyntaxError, NameError => e
        raise HecksPlayground::BluebookLoadError, "Failed to load domain '#{domain.name}': #{e.message}"
      end
    ...
    # auto-discovery that HecksPlayground.boot performs for file-based projects.
