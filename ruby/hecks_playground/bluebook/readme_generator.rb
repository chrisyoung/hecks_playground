  # HecksPlayground::ReadmeGenerator
  #
...
module HecksPlayground
  class ReadmeGenerator
    ...
    def generate
      template_path = File.join(@root, "hecks_playground_docs/readme_template.md")
    ...
    # under lib/hecks_playground/validation_rules/. Extracts the class name and first
    ...
    def validation_rules
      rules = []
      Dir[File.join(@root, "lib/hecks_playground/validation_rules/**/*.rb")].sort.each do |f|
        lines = File.readlines(f)
        next unless lines.first&.start_with?("# HecksPlayground::")
    ...
    # under lib/hecks_playground_cli/commands/. Extracts the command name from the
    # filename and description from the doc comment header.
    #
    # @return [String] markdown table with Command and Description columns
    def cli_commands
      commands = []
      Dir[File.join(@root, "lib/hecks_playground_cli/commands/*.rb")].sort.each do |f|
        lines = File.readlines(f)
        next unless lines.first&.start_with?("# HecksPlayground::")
        ...
        commands << "| `hecks_playground #{name}` | #{desc} |"
      end
      "| Command | Description |\n|---|---|\n#{commands.join("\n")}"
    end
    ...
    #
    # @return [HecksPlayground::BluebookModel::Domain, nil] the loaded domain, or nil
