module HecksPlayground
  module BluebookModel
    ...
    # HecksPlayground::BluebookModel::Structure::Domain
    #
    ...
    class Domain
      ...
      # @return [Array<String>] autoload entry point file names (e.g., ["hecks_playground_persist", "hecks_playground_mongodb"])
      ...
      def module_name
        HecksPlayground::Utils.sanitize_constant(name)
      end
      ...
      def gem_name
        HecksPlayground::Utils.underscore(module_name) + "_domain"
      end
      ...
      def describe
        ...
        aggregates.each do |agg|
          attrs = agg.attributes.map { |a| "#{a.name}: #{HecksPlayground::Utils.type_label(a)}" }.join(", ")
          lines << "  #{agg.name} (#{attrs})"
          agg.commands.each_with_index do |cmd, i|
            event = agg.events[i]
            cmd_attrs = cmd.attributes.map { |a| "#{a.name}: #{HecksPlayground::Utils.type_label(a)}" }.join(", ")
        ...
        flow_text = HecksPlayground::FlowGenerator.new(self).generate_text
      ...
      # command names, event names, etc.) to stdout. Delegates to HecksPlayground::BluebookGlossary.
      #
      # @return [nil]
      def glossary
        HecksPlayground::BluebookGlossary.new(self).print
      end
      ...
      def to_mermaid
        HecksPlayground::BluebookVisualizer.new(self).generate
      end
      ...
      # @return [nil]
      def visualize
        HecksPlayground::BluebookVisualizer.new(self).print
      end
      ...
      def flows
        HecksPlayground::FlowGenerator.new(self).generate_text
      end
      ...
      def flows_mermaid
        HecksPlayground::FlowGenerator.new(self).generate_mermaid
      end
      ...
      def validate_version!(v)
        ...
        raise HecksPlayground::InvalidBluebookVersion,
