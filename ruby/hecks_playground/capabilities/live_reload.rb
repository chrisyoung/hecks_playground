# HecksPlayground::Capabilities::LiveReload
#
# Live-reload capability for the HecksPlayground framework. Watches .bluebook,
...
#
# World config:
#   HecksPlayground.world "MyApp" do
#     live_reload do
#       watch_dirs ["hecks_playground", "lib"]
#       debounce   0.5
#     end
#   end
#
...
module HecksPlayground
  module Capabilities
    ...
    module LiveReload
      ...
      #
      # @param runtime [Object] the booted HecksPlayground runtime
      # @return [HecksPlayground::Capabilities::LiveReload::Watcher]
      def self.apply(runtime)
        config     = world_config(runtime)
        watch_dirs = Array(config[:watch_dirs] || ["hecks_playground"])
...
HecksPlayground.capability :live_reload do
  description "Hot-reload domain on .bluebook file changes"
  direction :driving
  config do
    watch_dirs ["hecks_playground"], desc: "Directories to watch"
    debounce 0.5, desc: "Debounce interval in seconds"
  end
  on_apply do |runtime|
    HecksPlayground::Capabilities::LiveReload.apply(runtime)
  end
end
