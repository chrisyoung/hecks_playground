# HecksPlayground::Capabilities::ProjectDiscovery::Bridge
#
# Discovers and boots HecksPlayground apps, reads their domain IR,
    ...
    module ProjectDiscovery
      class Bridge
        ...
        def open_project(path)
          ...
          hecks_playground_dir = File.join(path, "hecks_playground")
          bluebooks = Dir.glob(File.join(hecks_playground_dir, "*.bluebook")).sort
        ...
        def discover(search_path)
          Dir.glob(File.join(search_path, "**/hecks_playground/*.bluebook"))
        ...
        def discover_files(path)
          hecks_playground_dir = File.join(path, "hecks_playground")
          hecks_playground_files = Dir.glob(File.join(hecks_playground_dir, "*")).map { |f| { name: File.basename(f), path: f } }
          rb_files = Dir.glob(File.join(path, "*.rb")).map { |f| { name: File.basename(f), path: f } }
          hecks_playground_files + rb_files
        end
      end
    end
  end
