# HecksPlayground::Capabilities::Readme::Generator
#
# Builds HECKS_PLAYGROUND_README.md content from the runtime's domain IR,
    ...
    module Readme
      ...
      class Generator
        ...
        def capabilities_section
          ...
          lines = ["## Capabilities\n"]
          caps.each do |cap|
            meta = HecksPlayground.capability_meta[cap]
        ...
        def world_config_section
          ...
          lines = ["## World Config\n", "```ruby", "HecksPlayground.world \"#{world.name}\" do"]
          ...
          unconfigured = caps.select { |c| HecksPlayground.capability_config(c).any? } - configured
          unless unconfigured.empty?
            lines << ""
            lines << "### Available config (not yet configured)\n"
            unconfigured.each do |cap|
              lines << HecksPlayground.capability_config_template(cap)
              lines << ""
            end
          end
          lines.join("\n")
