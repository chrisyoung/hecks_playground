# HecksPlayground::Capabilities::WebClientState::JsGenerator
#
  ...
  module Capabilities
    module WebClientState
      class JsGenerator
        ...
        def generate
          <<~JS
            // HecksPlayground Client State — generated from #{@domain.name}
            // Optimistic dispatch + localStorage + correlation dedup
            (function() {
              "use strict";

              var STORAGE_KEY = "hecks_playground_state_#{@domain.name.downcase}";
              var pending = {};
              var counter = 0;
              ...
              function optimisticDispatch(aggregate, command, args) {
                ...
                if (window.HecksPlayground && window.HecksPlayground.handlers) {
                  var key = aggregate + "." + command;
                  if (window.HecksPlayground.handlers[key]) {
                    localResult = window.HecksPlayground.dispatch(aggregate, command, args);
                  }
                }
              ...
              // Replace HecksPlayground.dispatch with optimistic version
              var origDispatch = window.HecksPlayground ? window.HecksPlayground.dispatch : null;
              if (window.HecksPlayground) {
                window.HecksPlayground.optimisticDispatch = optimisticDispatch;
              }
