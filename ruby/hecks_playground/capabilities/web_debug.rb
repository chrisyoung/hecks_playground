# HecksPlayground::Capabilities::WebDebug
#
  ...
  module Capabilities
    module WebDebug
      ...
      def self.mount_routes(runtime)
        ...
        adapter.mount("/hecks_playground/screenshots") do |_req, res|
          res["Content-Type"] = "application/json"
          res.body = JSON.generate({ count: buffer.count, latest: buffer.latest })
        end

        js = CLIENT_JS
        adapter.mount("/hecks_playground/debug.js") do |_req, res|
...
HecksPlayground.capability :web_debug do
  description "Visual debugging — screenshot capture, event inspection"
  direction :driving
  on_apply do |runtime|
    HecksPlayground::Capabilities::WebDebug.apply(runtime)
  end
end
