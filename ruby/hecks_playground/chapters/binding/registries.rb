# = HecksPlayground::Binding::RegistriesChapter
#
# Self-describing sub-chapter for all HecksPlayground registries: domain,
# adapter, extension, capability, grammar, target, validation,
# dump format, thread context, and cross-domain.
#
#   HecksPlayground::Binding::RegistriesChapter.define(builder)
#
module HecksPlayground
  module Chapters
    ...
    # HecksPlayground::Binding::RegistriesChapter
    #
    # Bluebook sub-chapter defining all HecksPlayground registry aggregates: domain, adapter, extension, capability, and more.
    #
      module RegistriesChapter
        def self.define(b)
          ...
          b.aggregate "AdapterRegistryMethods", "Lazy registry methods for adapter types (memory, sqlite, etc.) extended onto HecksPlayground" do
            command("RegisterAdapter") { attribute :name, String }
          end

          b.aggregate "CapabilityRegistryMethods", "Lazy registry methods for domain capabilities extended onto HecksPlayground" do
            command("RegisterCapability") { attribute :name, String }
          end

          b.aggregate "DomainRegistryMethods", "Lazy registry for domain IR caching and load strategy extended onto HecksPlayground" do
            command("SetLoadStrategy") { attribute :strategy, String }
          end

          b.aggregate "ExtensionRegistryMethods", "Lazy registry for driven/driving extension hooks and metadata extended onto HecksPlayground" do
            command("DescribeExtension") { attribute :name, String; attribute :adapter_type, String }
          end

          b.aggregate "TargetRegistryMethods", "Lazy registry for build targets (ruby, go, rails) extended onto HecksPlayground" do
            command("RegisterTarget") { attribute :name, String }
          end

          b.aggregate "ValidationRegistryMethods", "Lazy registry for domain validation rule classes extended onto HecksPlayground" do
            command("RegisterRule") { attribute :rule_class, String }
          end

          b.aggregate "DumpFormatRegistryMethods", "Lazy registry for dump/serialization formats (schema, swagger) extended onto HecksPlayground" do
            command("RegisterFormat") { attribute :name, String; attribute :desc, String }
          end

          b.aggregate "GrammarRegistryMethods", "Lazy registry for modeling vocabularies (BlueBook, GameBook) extended onto HecksPlayground" do
          ...
          b.aggregate "CrossDomainMethods", "Cross-domain queries, views, and shared event bus methods extended onto HecksPlayground" do
            command("RegisterQuery") { attribute :name, String }
          end

          b.aggregate "ThreadContextMethods", "Thread-local tenant, actor, and current_user context methods extended onto HecksPlayground" do
