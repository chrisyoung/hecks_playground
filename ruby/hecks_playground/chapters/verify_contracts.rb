# HecksPlayground::Chapters::ContractVerifier
#
...
# data — HecksPlayground verifies itself.
#
#   HecksPlayground::Chapters::ContractVerifier.run
#   HecksPlayground::Chapters::ContractVerifier.run(format: :documentation)
#
    ...
    module ContractVerifier
      ...
      def self.verify_aggregate(result, format, domain, agg)
        ...
        check(result, format, ctx, "AggregateContract.rules") do
          rules = HecksPlayground::Conventions::AggregateContract.rules(agg)
          raise "no standard_fields" unless rules[:standard_fields]&.any?
        end

        check(result, format, ctx, "AggregateContract.partition_commands") do
          creates, updates = HecksPlayground::Conventions::AggregateContract.partition_commands(agg)
        ...
        check(result, format, ctx, "Names.slug + snake") do
          raise "blank slug" if HecksPlayground::Conventions::Names.bluebook_aggregate_slug(agg.name).to_s.empty?
          raise "blank snake" if HecksPlayground::Conventions::Names.bluebook_snake_name(agg.name).to_s.empty?
        end
      ...
      def self.verify_commands(result, format, domain, agg)
        agg.commands.each do |cmd|
          ...
          check(result, format, ctx, "CommandContract.method_name") do
            method = HecksPlayground::Conventions::CommandContract.method_name(cmd.name, agg.name)
            raise "not a symbol" unless method.is_a?(Symbol)
            raise "empty" if method.to_s.empty?
          end

          slug = HecksPlayground::Conventions::Names.bluebook_aggregate_slug(agg.name)
          cmd_snake = HecksPlayground::Utils.underscore(cmd.name)

          check(result, format, ctx, "RouteContract.form_path") do
            path = HecksPlayground::Conventions::RouteContract.form_path(slug, cmd_snake)
          ...
          check(result, format, ctx, "RouteContract.submit_path") do
            path = HecksPlayground::Conventions::RouteContract.submit_path(slug, cmd_snake)
            raise "blank" if path.to_s.empty?
          end
        end
      end
      ...
      def self.verify_events(result, format, domain, agg)
        agg.events.each do |event|
          ...
          check(result, format, ctx, "EventContract.REQUIRED_FIELDS") do
            HecksPlayground::Conventions::EventContract::REQUIRED_FIELDS.each do |field, types|
        ...
        agg.attributes.each do |attr|
          %i[go sql json openapi typescript].each do |target|
            check(result, format, "#{domain.name}/#{agg.name}.#{attr.name}",
              ...
              mapped = HecksPlayground::Conventions::TypeContract.for(target, attr.type)
      ...
      def self.verify_lifecycle(result, format, domain, agg)
        ...
        check(result, format, "#{domain.name}/#{agg.name}", "AggregateContract.lifecycle") do
          lc = HecksPlayground::Conventions::AggregateContract.rules(agg)[:lifecycle]
      ...
      def self.verify_validations(result, format, domain, agg)
        ...
        check(result, format, "#{domain.name}/#{agg.name}", "AggregateContract.validations") do
          HecksPlayground::Conventions::AggregateContract.rules(agg)[:validations].each do |v|
      ...
      def self.verify_display(result, format, domain, agg)
        ...
        check(result, format, ctx, "DisplayContract.aggregate_summary") do
          summary = HecksPlayground::Conventions::DisplayContract.aggregate_summary(agg)
        ...
        agg.attributes.each do |attr|
          check(result, format, "#{ctx}.#{attr.name}", "DisplayContract.cell_expression(:ruby)") do
            expr = HecksPlayground::Conventions::DisplayContract.cell_expression(attr, "obj", lang: :ruby, domain: domain)
            raise "nil expression" if expr.nil?
          end

          check(result, format, "#{ctx}.#{attr.name}", "DisplayContract.cell_expression(:go)") do
            expr = HecksPlayground::Conventions::DisplayContract.cell_expression(attr, "obj", lang: :go, domain: domain)
            raise "nil expression" if expr.nil?
          end
        end
        ...
        if agg.lifecycle
          check(result, format, ctx, "DisplayContract.lifecycle_transitions") do
            labels = HecksPlayground::Conventions::DisplayContract.lifecycle_transitions(agg.lifecycle)
      ...
      def self.verify_ui_labels(result, format, domain, agg)
        ...
        check(result, format, ctx, "UILabelContract.label") do
          label = HecksPlayground::Conventions::UILabelContract.label(agg.name)
          raise "blank" if label.to_s.empty?
        end

        check(result, format, ctx, "UILabelContract.plural_label") do
          label = HecksPlayground::Conventions::UILabelContract.plural_label(agg.name)
          raise "blank" if label.to_s.empty?
        end

        agg.attributes.each do |attr|
          check(result, format, "#{ctx}.#{attr.name}", "UILabelContract.label") do
            label = HecksPlayground::Conventions::UILabelContract.label(attr.name)
            raise "blank" if label.to_s.empty?
          end
        end
        ...
        agg.commands.each do |cmd|
          check(result, format, "#{ctx}/#{cmd.name}", "UILabelContract.label") do
            label = HecksPlayground::Conventions::UILabelContract.label(cmd.name)
            raise "blank" if label.to_s.empty?
          end
        end
      end
      ...
      def self.verify_view_contracts(result, format, domain, agg)
        ...
        HecksPlayground::Conventions::ViewContract.all.each do |template_name, contract|
          ...
          check(result, format, ctx, "ViewContract.go_struct(:#{template_name})") do
            struct = HecksPlayground::Conventions::ViewContract.go_struct(template_name, contract[:fields])
      ...
      def self.verify_form_parsing(result, format, domain, agg)
        agg.attributes.each do |attr|
          go_type = HecksPlayground::Conventions::TypeContract.for(:go, attr.type)

          check(result, format, "#{domain.name}/#{agg.name}.#{attr.name}",
            "FormParsingContract.input_type(#{go_type})") do
            input = HecksPlayground::Conventions::FormParsingContract.input_type(go_type)
            raise "nil input type" if input.nil?
          end

          check(result, format, "#{domain.name}/#{agg.name}.#{attr.name}",
            "FormParsingContract.go_parse_line") do
            go_field = HecksPlayground::Utils.sanitize_constant(attr.name)
            line = HecksPlayground::Conventions::FormParsingContract.go_parse_line(attr.name.to_s, go_field, go_type)
            raise "blank" if line.to_s.empty?
          end

          check(result, format, "#{domain.name}/#{agg.name}.#{attr.name}",
            "FormParsingContract.ruby_coerce") do
            ruby_type = HecksPlayground::Conventions::TypeContract.for(:json, attr.type)
            expr = HecksPlayground::Conventions::FormParsingContract.ruby_coerce(attr.name.to_s, ruby_type)
      ...
      def self.verify_event_log(result, format, domain, agg)
        ...
        check(result, format, ctx, "EventLogContract.FIELDS") do
          raise "no fields" unless HecksPlayground::Conventions::EventLogContract::FIELDS.any?
        end

        check(result, format, ctx, "EventLogContract.go_struct") do
          struct = HecksPlayground::Conventions::EventLogContract.go_struct
        ...
        check(result, format, ctx, "EventLogContract.go_mapper") do
          mapper = HecksPlayground::Conventions::EventLogContract.go_mapper
          raise "blank" if mapper.to_s.empty?
        end

        check(result, format, ctx, "EventLogContract.ruby_mapper") do
          mapper = HecksPlayground::Conventions::EventLogContract.ruby_mapper
          raise "blank" if mapper.to_s.empty?
        end
      end
