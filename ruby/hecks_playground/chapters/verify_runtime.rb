# HecksPlayground::Chapters::RuntimeVerifier
#
...
#
#   HecksPlayground::Chapters::RuntimeVerifier.run
#   HecksPlayground::Chapters::RuntimeVerifier.run(format: :documentation)
#
  ...
  module Chapters
    module RuntimeVerifier
      ...
      def self.run(format: :progress)
        ...
        app = HecksPlayground.boot(pizzas_dir)
      ...
      def self.verify_round_trip(result, format, app)
        check(result, format, "Serialization", "DslSerializer round-trip") do
          original = app.domain
          dsl_source = HecksPlayground::DslSerializer.new(original).serialize
          restored = HecksPlayground::DSL::AggregateBuilder::VoTypeResolution.with_vo_constants { eval(dsl_source) }
          diff = HecksPlayground::Conventions::MigrationContract.diff(original, restored)
          unless diff[:valid]
      ...
      def self.as_role(name)
        HecksPlayground.current_role = name
        HecksPlayground.actor = OpenStruct.new(role: name)
      end
