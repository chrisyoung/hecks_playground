# HecksPlayground::Compiler::BinaryCompiler
#
# Orchestrates compilation of the HecksPlayground framework into a single
...
#   compiler.compile(output: "hecks_playground_v0")
#
module HecksPlayground
  module Compiler
    class BinaryCompiler
      ...
      # Compiles all HecksPlayground source into a bundled executable using
      # static analysis for file ordering (no runtime introspection).
      #
      # @param output [String] output file path (default: "hecks_playground_v0")
      # @return [String] absolute path to the compiled binary
      def compile(output: "hecks_playground_v0", trace: false)
        files = SourceAnalyzer.analyze(lib_root: lib_root, trace: trace)

        if files.empty?
          raise "No HecksPlayground source files found under #{lib_root}."
        end
