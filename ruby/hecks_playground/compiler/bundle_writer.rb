# HecksPlayground::Compiler::BundleWriter
#
...
#
#   HecksPlayground::Compiler::BundleWriter.write(files, output: "hecks_playground_v0")
#
module HecksPlayground
  module Compiler
    module BundleWriter
      ...
      BANNER  = <<~RUBY
        #
        # HecksPlayground v0 — self-compiled binary
      ...
      def self.write_stdlib_requires(io)
        ...
        io.puts "require \"set\""
        io.puts "$HECKS_PLAYGROUND_V0 = true"
        io.puts ""
      end

      def self.write_loaded_features(io, files)
        io.puts "# Remove hecks_playground gem paths from $LOAD_PATH to prevent"
        io.puts "# double-loading from the original source on disk."
        io.puts "$LOAD_PATH.reject! { |p| p.include?(\"hecks_playground\") }"
        io.puts ""
      ...
      def self.write_entrypoint(io)
        io.puts <<~'RUBY'
          # --- HecksPlayground v0 entrypoint ---
          if __FILE__ == $0
            command = ARGV.shift
            case command
            when "boot"
              path = ARGV.shift || "."
              app = HecksPlayground.boot(File.expand_path(path))
              puts "HecksPlayground v0: booted #{app.domain.name} from #{path}"
            when "version"
              puts "HecksPlayground v0 (self-compiled)"
              puts "Version: #{HecksPlayground::VERSION}"
            when "self-test"
              puts "HecksPlayground v0 self-test:"
              puts "  Version: #{HecksPlayground::VERSION}"
              puts "  Modules: #{HecksPlayground.constants.size}"
              puts "  Targets: #{HecksPlayground.target_registry.keys.join(', ')}"
              puts "  Status: OK"
            else
              puts "HecksPlayground v0 -- self-compiled domain compiler"
              puts ""
              puts "Usage:"
              puts "  hecks_playground_v0 boot [path]     Boot a domain from path"
              puts "  hecks_playground_v0 version         Show version info"
              puts "  hecks_playground_v0 self-test       Run self-consistency check"
            end
          end
        RUBY
      end
