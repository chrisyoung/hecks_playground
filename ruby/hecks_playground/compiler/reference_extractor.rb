# HecksPlayground::Compiler::ReferenceExtractor
#
...
#   refs  # => ["HecksPlayground::Compiler", "Prism::Visitor", "JSON"]
#
require "prism"
...
module HecksPlayground
  module Compiler
    class ReferenceExtractor < Prism::Visitor
      ...
      # Extracts all constant references and HecksPlayground.method calls from source.
      #
      ...
      def visit_call_node(node)
        ...
        # Detect HecksPlayground.method_name calls for Bluebook IR resolution
        if hecks_playground_method_call?(node)
      ...
      # Detects calls like HecksPlayground.describe_extension, HecksPlayground.register_xxx
      def hecks_playground_method_call?(node)
        return false unless node.receiver
        receiver_name = resolve_constant_path(node.receiver)
        receiver_name == "HecksPlayground" && node.name.to_s != "new"
      end
