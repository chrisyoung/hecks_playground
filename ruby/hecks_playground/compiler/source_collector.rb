# HecksPlayground::Compiler::SourceCollector
#
# Collects all HecksPlayground framework source files in load order by
# introspecting $LOADED_FEATURES after requiring "hecks_playground". Returns
# the exact order Ruby resolved dependencies.
#
#   files = HecksPlayground::Compiler::SourceCollector.collect(lib_root: "lib")
#   files.each { |path| puts path }
#
module HecksPlayground
  module Compiler
    module SourceCollector
      # Returns an ordered list of absolute paths for all HecksPlayground source
      ...
      def self.trigger_lazy_loads(lib_root)
        ...
        HecksPlayground.boot(pizzas) if File.directory?(pizzas)
      rescue => e
