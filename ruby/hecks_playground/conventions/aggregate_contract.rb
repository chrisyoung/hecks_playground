# = HecksPlayground::Conventions::AggregateContract
#
...
#
#   rules = HecksPlayground::Conventions::AggregateContract.rules(aggregate_ir)
...
module HecksPlayground::Conventions
  ...
  module AggregateContract
    ...
    def self.partition_commands(agg)
      agg_snake = HecksPlayground::Utils.underscore(agg.name)
    ...
    class << self
      ...
      def extract_enums(agg)
        attrs = agg.attributes.reject { |attr|
          HecksPlayground::Utils::RESERVED_AGGREGATE_ATTRS.include?(attr.name.to_s)
        }
