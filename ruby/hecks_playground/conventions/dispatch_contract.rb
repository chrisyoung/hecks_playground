# = HecksPlayground::Conventions::DispatchContract
#
    ...
    def self.build_whitelist(domain)
      domain.aggregates.each_with_object({}) do |agg, wl|
        ...
        agg.commands.each do |cmd|
          allowed << HecksPlayground::Conventions::CommandContract.method_name(cmd.name, agg.name)
        end
        agg.queries.each do |query|
          allowed << HecksPlayground::Conventions::Names.bluebook_snake_name(query.name).to_sym
        end
