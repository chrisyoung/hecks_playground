# = HecksPlayground::Conventions::DisplayContract
#
...
#
#   HecksPlayground::Conventions::DisplayContract.cell_expression(attr, "obj")
#   HecksPlayground::Conventions::DisplayContract.lifecycle_transitions(lifecycle)
#   HecksPlayground::Conventions::DisplayContract.aggregate_summary(agg)
#   HecksPlayground::Conventions::DisplayContract.policy_labels(domain)
#   HecksPlayground::Conventions::DisplayContract.available_roles(domain)
#
module HecksPlayground::Conventions
  ...
  module DisplayContract
    ...
    def self.find_referenced_aggregate(attr, domain)
      base = attr.name.to_s.sub(/_id\z/, "")
      pascal = HecksPlayground::Utils.sanitize_constant(base)
    ...
    def self.lifecycle_transitions(lc)
      lc.transitions.map do |cmd_name, _|
        label = UILabelContract.label(HecksPlayground::Utils.underscore(cmd_name))
    ...
    def self.home_aggregate_data(agg, plural)
      user_attrs = agg.attributes.reject { |attr|
        HecksPlayground::Utils::RESERVED_AGGREGATE_ATTRS.include?(attr.name.to_s) || !attr.visible?
      }
      {
    ...
    GoFieldName = ->(name) { HecksPlayground::Utils.sanitize_constant(name) }
  end
end
