# = HecksPlayground::Conventions::ExtensionContract
#
# Enforces the required interface for HecksPlayground extensions. Every extension
# must declare metadata via +HecksPlayground.describe_extension+ and register a
# boot hook via +HecksPlayground.register_extension+. The boot hook must accept
...
#
#   result = HecksPlayground::Conventions::ExtensionContract.validate(:logging)
#   result[:valid]   # => true
#   result[:missing] # => []
#
#   result = HecksPlayground::Conventions::ExtensionContract.validate(:bogus)
#   result[:valid]   # => false
...
module HecksPlayground::Conventions
  ...
  module ExtensionContract
    ...
    def self.all_extension_names
      names = []
      names.concat(HecksPlayground.extension_meta.map(&:first)) if HecksPlayground.respond_to?(:extension_meta)
      names.concat(HecksPlayground.extension_registry.map(&:first)) if HecksPlayground.respond_to?(:extension_registry)
      names.uniq.sort
    end
    ...
    class << self
      ...
      def check_metadata(name, missing)
        return nil unless HecksPlayground.respond_to?(:extension_meta)
      ...
      def check_boot_hook(name, missing)
        return unless HecksPlayground.respond_to?(:extension_registry)

        hook = HecksPlayground.extension_registry[name]
        unless hook
