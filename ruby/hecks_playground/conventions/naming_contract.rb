# = HecksPlayground::Conventions::Names
#
# Single source of truth for all naming conventions in HecksPlayground.
# Include in any class/module for bare method calls:
#
#   include HecksPlayground::Conventions::Names
  ...
  #
    module Names
      ...
      def bluebook_module_name(name)
        HecksPlayground::Utils.sanitize_constant(name) + "Bluebook"
      end

      # "Pizzas" → "pizzas_domain"
      def bluebook_gem_name(name)
        HecksPlayground::Utils.underscore(HecksPlayground::Utils.sanitize_constant(name)) + "_bluebook"
      end
      ...
      def bluebook_snake_name(name)
        HecksPlayground::Utils.underscore(name)
      end

      # "Pizza" → "pizzas"
      def bluebook_aggregate_slug(name)
        s = HecksPlayground::Utils.underscore(HecksPlayground::Utils.sanitize_constant(name))
        s.end_with?("s") ? s : s + "s"
      end
      ...
      def bluebook_command_name(verb, aggregate_name)
        ...
        if parts.size == 1
          parts.first.capitalize + HecksPlayground::Utils.sanitize_constant(aggregate_name)
        else
      ...
      def domain_command_method(cmd_name, agg_name)
        HecksPlayground::Conventions::CommandContract.method_name(cmd_name, agg_name)
      end
      ...
      def bluebook_output_dir(domain_name)
        HecksPlayground::Utils.underscore(HecksPlayground::Utils.sanitize_constant(domain_name)) + "_bluebook"
      end
    end
end
