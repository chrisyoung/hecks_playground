# HecksPlayground::DomainVersioning
#
# Manages domain interface version snapshots. Snapshots are copies of the
# domain DSL file stored in db/hecks_playground_versions/ with a metadata header.
# Supports tagging, listing, loading, and diffing version snapshots.
#
#   HecksPlayground::DomainVersioning.tag("2.1.0", domain, base_dir: Dir.pwd)
#   versions = HecksPlayground::DomainVersioning.log(base_dir: Dir.pwd)
#   domain   = HecksPlayground::DomainVersioning.load_version("2.1.0", base_dir: Dir.pwd)
#
...
module HecksPlayground
  # HecksPlayground::DomainVersioning
  #
  # Manages domain interface version snapshots stored in db/hecks_playground_versions/ for diffing and tagging.
  #
  module DomainVersioning
    VERSIONS_DIR = "db/hecks_playground_versions"

    # Tag the current domain as a named version snapshot.
    #
    # @param version [String] semantic version label (e.g. "2.1.0")
    # @param domain [HecksPlayground::BluebookModel::Domain] the domain to snapshot
    ...
    def self.tag(version, domain, base_dir: Dir.pwd)
      ...
      header = "# HecksPlayground domain snapshot\n# version: #{version}\n# tagged_at: #{Date.today}\n"
    ...
    # @param base_dir [String] project root directory
    # @return [HecksPlayground::BluebookModel::Domain, nil]
