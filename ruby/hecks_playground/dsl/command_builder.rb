# [antibody-exempt: ruby/hecks_playground/dsl/command_builder.rb — kernel-floor DSL
...
#  justification as the rest of ruby/hecks_playground/dsl/. Edit for i250 :
#  `emits` accepts `identified_by:` kwarg, stored on the built Command IR.]

module HecksPlayground
  module DSL

    # HecksPlayground::DSL::CommandBuilder
    #
    ...
    class CommandBuilder
      ...
      def then_set(field, positional = nil, to: nil, append: nil, append_unique: nil, remove: nil, increment: nil, decrement: nil, multiply: nil, clamp: nil, decay: nil, from: nil)
        op, val = if !to.nil? then [:set, to]
                  elsif append then [:append, append]
                  elsif append_unique then [:append_unique, append_unique]
      ...
      def reference_to(type, as: nil, role: nil, validate: :exists)
        raise ArgumentError, "reference_to requires a constant, not a string: #{type.inspect}" if type.class == String && HecksPlayground::DSL::TypeName.match?(type)
        type_str = type.to_s
