...
#
# Future gem: hecks_playground_audit
#
# Usage:
#   require "hecks_playground_audit"
#
#   app = HecksPlayground.load(domain)
#   audit = HecksPlayground::Audit.new(app.event_bus)
#
...
class HecksPlayground::Audit
  ...
  #
  # @param event_bus [HecksPlayground::EventBus] the event bus to subscribe to;
  ...
  def around_command(command, next_handler, actor: nil, tenant: nil)
    @pending_context = {
      command: HecksPlayground::Utils.const_short_name(command),
      actor: actor,
      tenant: tenant
    }
    next_handler.call
  ...
  def record(event)
    entry = {
      ...
      timestamp: Time.now,
      event_name: HecksPlayground::Utils.const_short_name(event),
...
HecksPlayground.describe_extension(:audit,
  description: "Immutable audit trail for every domain event",
  adapter_type: :driven,
  config: {},
  wires_to: :event_bus)

HecksPlayground.register_extension(:audit) do |_domain_mod, _domain, runtime|
  bus = HecksPlayground.event_bus || runtime.event_bus
  audit = HecksPlayground::Audit.new(bus)
  HecksPlayground.instance_variable_set(:@_audit, audit)
  HecksPlayground.define_singleton_method(:audit_log) { @_audit.log }
  runtime.use(:audit) do |cmd, nxt|
    audit.around_command(cmd, nxt, actor: HecksPlayground.actor&.respond_to?(:role) ? HecksPlayground.actor.role : nil)
  end
end
