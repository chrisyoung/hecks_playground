# HecksAuth
#
# Authentication and authorization extension for HecksPlayground domains. Reads
...
# the allowed list. Raises +HecksPlayground::Unauthenticated+ if no actor is set,
# or +HecksPlayground::Unauthorized+ if the actor's role is not permitted.
#
# Future gem: hecks_playground_auth
#
#   # Gemfile
#   gem "cats_domain"
...
#   HecksPlayground.actor = OpenStruct.new(role: "Admin")
#   Cat.adopt(name: "Whiskers")  # checks actor role against DSL
#
HecksPlayground.describe_extension(:auth,
...
HecksPlayground.register_extension(:auth) do |domain_mod, domain, runtime, **opts|
  ...
  actor_map = HecksPlayground::Conventions::Names.actor_roles_for(domain, domain_mod)
  ...
  # 3. If an entry exists, checks HecksPlayground.actor is set (raises Unauthenticated if not)
  ...
  runtime.use :auth do |command, next_handler|
    ...
    if required_roles && !Thread.current[:_hecks_playground_skip_role_check]
      actor = HecksPlayground.actor
      raise HecksPlayground::Unauthenticated, "No actor set. Call HecksPlayground.actor = user before running #{HecksPlayground::Utils.const_short_name(command)}" unless actor
      role = actor.respond_to?(:role) ? actor.role.to_s : actor.to_s
      unless required_roles.include?(role)
        raise HecksPlayground::Unauthorized, "Actor '#{role}' is not authorized for #{HecksPlayground::Utils.const_short_name(command)}. Required: #{required_roles.join(', ')}"
      end
    end
