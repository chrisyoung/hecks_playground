# HecksPlayground::Extensions::Claude
#
...
HecksPlayground.describe_extension(:claude,
...
HecksPlayground.register_extension(:claude) do |_domain_mod, _domain, _runtime|
  next unless defined?(HecksPlayground::Capabilities::ChatAgent)

  world = HecksPlayground.last_world
  config = world&.config_for(:claude) || {}
  model = config[:model] || "sonnet"
  max_tokens = config[:max_tokens] || 4096

  adapter = if config[:api_key] && !config[:api_key].to_s.empty?
    HecksPlayground::Extensions::ClaudeAdapter.new(
      api_key: config[:api_key], model: model, max_tokens: max_tokens
    )
  else
    HecksPlayground::Extensions::ClaudeCliAdapter.new(
      model: model, max_tokens: max_tokens
    )
  end

  HecksPlayground::Capabilities::ChatAgent.register_adapter(:claude, adapter)
end
