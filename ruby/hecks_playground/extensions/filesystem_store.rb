...
#
# JSON file persistence extension for HecksPlayground domains. Stores each aggregate
...
#
# Future gem: hecks_playground_filesystem_store
#
#   # Gemfile
#   gem "cats_domain"
#   gem "hecks_playground_filesystem_store"   # auto-wires at boot
#
#   # Or explicitly:
#   app = HecksPlayground.boot(__dir__, adapter: :filesystem)
#
HecksPlayground.describe_extension(:filesystem_store,
  description: "JSON file persistence",
  adapter_type: :driven,
  config: { data_dir: { default: "./data", desc: "Directory for JSON files" } },
  wires_to: :repository)

HecksPlayground.register_extension(:filesystem_store) do |domain_mod, domain, runtime|
  require "json"
  require "time"
  require "fileutils"

  data_dir = ENV.fetch("HECKS_PLAYGROUND_DATA_DIR", "./data")

  domain.aggregates.each do |agg|
    repo = HecksPlayground::FilesystemRepository.new(
      agg.name,
  ...
  # HecksPlayground::FilesystemRepository
  #
...
module HecksPlayground
  ...
  class FilesystemRepository
      ...
      unless conditions.empty?
        results = results.select do |obj|
          conditions.all? do |cond_key, cond_val|
            ...
            cond_val.is_a?(HecksPlayground::Querying::Operators::Operator) ? cond_val.match?(actual) : actual == cond_val
          end
        end
      end
      if order_key
    ...
    def serialize(obj)
      hash = { "id" => obj.id }
      if obj.class.respond_to?(:hecks_playground_attributes)
        obj.class.hecks_playground_attributes.each do |attr|
    ...
    def serialize_value(val)
      ...
      when ->(value) { value.class.respond_to?(:hecks_playground_attributes) }
        result = {}
        val.class.hecks_playground_attributes.each { |attr| result[attr.name.to_s] = serialize_value(val.send(attr.name)) }
        result
      else val
      end
    end
    ...
    def deserialize(data)
      attrs = {}
      if @aggregate_class.respond_to?(:hecks_playground_attributes)
        @aggregate_class.hecks_playground_attributes.each do |attr|
...
HecksPlayground.alias_extension(:filesystem, :filesystem_store)
