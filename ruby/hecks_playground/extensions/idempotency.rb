# HecksIdempotency
#
# Idempotency extension for the HecksPlayground command bus. Deduplicates retried
...
#
# Future gem: hecks_playground_idempotency
#
# Configuration via environment variable:
#   HECKS_PLAYGROUND_IDEMPOTENCY_TTL -- cache TTL in seconds (default: 300)
#
# Usage:
#   require "hecks_playground_idempotency"
#   app.run("CreatePizza", name: "Margherita")  # first call executes
#   app.run("CreatePizza", name: "Margherita")  # second call returns cached
#
HecksPlayground.describe_extension(:idempotency,
  description: "Idempotent command execution via dedup keys",
  adapter_type: :driven,
  config: {
    HECKS_PLAYGROUND_IDEMPOTENCY_TTL: { default: 300, desc: "Cache TTL in seconds" }
  },
  wires_to: :command_bus)

HecksPlayground.register_extension(:idempotency) do |_domain_mod, _domain, runtime|
  cache = {}
  ttl = ENV.fetch("HECKS_PLAYGROUND_IDEMPOTENCY_TTL", "300").to_i
