...
#
# Emitted entries are appended to +HecksPlayground.metric_log+ by default. When a
# custom sink is registered via +HecksPlayground.metric_sink=+, each entry is also
# forwarded to +sink.call(entry)+.
#
# Future gem: hecks_playground_metrics
#
...
HecksPlayground.describe_extension(:metrics,
  description: "Metric change tracking for tagged aggregate attributes",
  adapter_type: :driven,
  config: {},
  wires_to: :command_bus)

HecksPlayground.register_extension(:metrics) do |domain_mod, domain, runtime|
  # Build a lookup: aggregate_name => [metric_field_symbols]
  hecksagon = HecksPlayground.last_hecksagon
  metric_lookup = {}
  domain.aggregates.each do |agg|
    fields = HecksPlayground::Metrics.metric_fields(hecksagon, agg.name)
    metric_lookup[agg.name] = fields unless fields.empty?
  end

  # Initialize global metric log and sink on HecksPlayground module (once).
  unless HecksPlayground.respond_to?(:metric_log)
    HecksPlayground.instance_variable_set(:@_metric_log, [])
    HecksPlayground.instance_variable_set(:@_metric_sink, nil)
    HecksPlayground.define_singleton_method(:metric_log) { @_metric_log }
    HecksPlayground.define_singleton_method(:metric_sink=) { |s| @_metric_sink = s }
    HecksPlayground.define_singleton_method(:metric_sink) { @_metric_sink }
  end
  ...
  runtime.use :metrics do |command, next_handler|
    ...
    if agg_name
      ...
      entity_before = HecksPlayground::Metrics.find_before(command, repo) if repo
      before_values = HecksPlayground::Metrics.read_values(entity_before, fields)
    end
    ...
    if agg_name
      ...
      after_values = HecksPlayground::Metrics.read_values(entity_after, fields)
      ...
      fields.each do |attr|
        ...
        HecksPlayground.metric_log << entry
        HecksPlayground.metric_sink&.call(entry)
      end
    end
