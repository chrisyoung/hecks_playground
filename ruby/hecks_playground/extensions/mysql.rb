# [antibody-exempt: ruby/hecks_playground/extensions/mysql.rb — kernel-floor Sequel + mysql2 bridge wired through HecksPlayground.register_extension(:mysql). Retires when the :sql_database adapter family ships as a bluebook with provider bindings.]
# HecksMysql
#
# MySQL persistence extension for HecksPlayground domains. Auto-wires when
...
#   HECKS_PLAYGROUND_DB_USER     -- database user (default: "root")
#   HECKS_PLAYGROUND_DB_PASSWORD -- database password (default: nil)
#
# Future gem: hecks_playground_mysql
#
#   # Gemfile
#   gem "cats_domain"
#   gem "hecks_playground_mysql"   # auto-wires MySQL
#
require "hecks_playground_persist"

HecksPlayground.describe_extension(:mysql,
...
HecksPlayground.register_extension(:mysql) do |domain_mod, domain, runtime|
  begin
    require "sequel"
  rescue LoadError => e
    raise LoadError,
      "HecksPlayground's :mysql extension requires the `sequel` and `mysql2` gems. " \
      "Add to your Gemfile :\n\n    gem \"sequel\"\n    gem \"mysql2\"\n\n" \
  ...
  db = Sequel.connect(adapter: :mysql2,
    ...
    user:     ENV.fetch("HECKS_PLAYGROUND_DB_USER", "root"),
