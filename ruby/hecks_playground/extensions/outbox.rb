...
#   HecksPlayground.outbox.entries.size     # => 1 (stored + already drained)
#   HecksPlayground.outbox.pending_count    # => 0 (poller drained it)
#   app.events.size               # => 1 (delivered via poller)
#
HecksPlayground.describe_extension(:outbox,
...
HecksPlayground.register_extension(:outbox) do |_domain_mod, _domain, runtime, **kwargs|
  ...
  outbox = HecksPlayground::Outbox::MemoryOutbox.new
  ...
  poller = HecksPlayground::Outbox::OutboxPoller.new(outbox, bus, publisher: original_publish)
  ...
  # Expose outbox and poller on the HecksPlayground module
  HecksPlayground.instance_variable_set(:@_outbox, outbox)
  HecksPlayground.instance_variable_set(:@_outbox_poller, poller)
  HecksPlayground.define_singleton_method(:outbox) { @_outbox }
  HecksPlayground.define_singleton_method(:outbox_poller) { @_outbox_poller }
end
