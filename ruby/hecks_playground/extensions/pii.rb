# HecksPlayground::PII
#
# PII (Personally Identifiable Information) protection extension for HecksPlayground
...
#
# Future gem: hecks_playground_pii
#
#   # DSL
...
module HecksPlayground::PII
  ...
  #
  # @example
  #   HecksPlayground::PII.mask("john@example.com")  # => "j**************m"
  #   HecksPlayground::PII.mask("Jo")                # => "[REDACTED]"
  #   HecksPlayground::PII.mask(nil)                 # => nil
  def self.mask(value)
...
HecksPlayground.describe_extension(:pii,
  description: "PII field encryption and masking",
  adapter_type: :driven,
  config: {},
  wires_to: :repository)

HecksPlayground.register_extension(:pii) do |domain_mod, domain, runtime|
  ...
  domain_mod.define_singleton_method(:erase_pii) do |entity_id|
    domain.aggregates.each do |agg|
      pii_names = HecksPlayground::PII.pii_fields(agg)
  ...
  domain_mod.define_singleton_method(:pii_fields) do
    domain.aggregates.each_with_object({}) do |agg, h|
      fields = HecksPlayground::PII.pii_fields(agg)
  ...
  if HecksPlayground.extension_registry[:audit]
    pii_lookup = {}
    domain.aggregates.each do |agg|
      agg.commands.each do |cmd|
        fqn = HecksPlayground::Conventions::Names.bluebook_command_fqn(domain_mod.name, agg.name, cmd.name)
        pii_names = HecksPlayground::PII.pii_fields(agg)
      ...
      if pii_names && domain_mod.respond_to?(:audit_log)
        ...
        if entry && entry[:attributes]
          pii_names.each do |name|
            entry[:attributes][name] = HecksPlayground::PII.mask(entry[:attributes][name]) if entry[:attributes].key?(name)
          end
        end
      end
      result
