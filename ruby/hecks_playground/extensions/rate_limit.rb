# HecksRateLimit
#
# Rate limiting extension for the HecksPlayground command bus. Uses a sliding window
# counter per actor to limit how many commands an actor can dispatch in
# a configurable time period. Only applies when +HecksPlayground.actor+ is set;
...
# limit, a +HecksPlayground::RateLimitExceeded+ error is raised.
#
# Future gem: hecks_playground_rate_limit
#
# Configuration via environment variables:
#   HECKS_PLAYGROUND_RATE_LIMIT  -- max commands per window (default: 60)
#   HECKS_PLAYGROUND_RATE_PERIOD -- window size in seconds (default: 60)
#
# Usage:
#   require "hecks_playground_rate_limit"
#   HecksPlayground.actor = current_user
#   app.run("CreatePizza", name: "Margherita")  # rate-limited per actor
#
HecksPlayground.describe_extension(:rate_limit,
  description: "Per-actor sliding window rate limiting",
  adapter_type: :driven,
  config: {
    HECKS_PLAYGROUND_RATE_LIMIT: { default: 60, desc: "Max commands per window" },
    HECKS_PLAYGROUND_RATE_PERIOD: { default: 60, desc: "Window size in seconds" }
  },
  wires_to: :command_bus)

HecksPlayground.register_extension(:rate_limit) do |_domain_mod, _domain, runtime|
  window = {}
  limit = ENV.fetch("HECKS_PLAYGROUND_RATE_LIMIT", "60").to_i
  period = ENV.fetch("HECKS_PLAYGROUND_RATE_PERIOD", "60").to_i

  # Register command bus middleware that enforces per-actor rate limits.
  #
  # For each command dispatched:
  # 1. Checks if HecksPlayground.actor is set; if not, skips rate limiting
  ...
  #    HecksPlayground::RateLimitExceeded with a descriptive message
  ...
  # @raise [HecksPlayground::RateLimitExceeded] when the actor exceeds the rate limit
  runtime.use :rate_limit do |command, next_handler|
    actor = HecksPlayground.actor
    if actor
      ...
      if window[key].size >= limit
        raise HecksPlayground::RateLimitExceeded, "Rate limit exceeded (#{limit}/#{period}s)"
      end
      window[key] << now
    end
    next_handler.call
