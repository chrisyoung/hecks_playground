...
# non-domain StandardError subclasses). Domain errors (HecksPlayground::Error
...
#   HECKS_PLAYGROUND_RETRY_MAX   -- maximum retry attempts (default: 3)
#   HECKS_PLAYGROUND_RETRY_DELAY -- base delay in seconds (default: 0.1)
#
# Future gem: hecks_playground_retry
#
#   require "hecks_playground_retry"
#   # Automatically registered -- transient failures will be retried
#   # up to HECKS_PLAYGROUND_RETRY_MAX times with exponential backoff.
#
HecksPlayground.describe_extension(:retry,
...
HecksPlayground.register_extension(:retry) do |domain_mod, domain, runtime|
  max_retries = ENV.fetch("HECKS_PLAYGROUND_RETRY_MAX", "3").to_i
  base_delay = ENV.fetch("HECKS_PLAYGROUND_RETRY_DELAY", "0.1").to_f
  ...
  # 2. If a HecksPlayground::Error (domain error) is raised, re-raises immediately
  ...
  # @raise [HecksPlayground::Error] domain errors are always re-raised immediately
