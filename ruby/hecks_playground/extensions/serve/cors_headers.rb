# HecksPlayground::HTTP::CorsHeaders
#
# Mixin that provides ENV-driven CORS origin logic for HecksPlayground HTTP servers.
...
#   HECKS_PLAYGROUND_ALLOW_ALL_ORIGINS=true          → emits `*`
#   HECKS_PLAYGROUND_CORS_ORIGIN=https://app.example.com → emits that value
...
module HecksPlayground
  module HTTP
    # HecksPlayground::HTTP::CorsHeaders
    #
    # Mixin providing ENV-driven CORS origin logic for HecksPlayground HTTP servers.
    #
    module CorsHeaders
      # Return the CORS origin value based on ENV configuration.
      #
      # Checks HECKS_PLAYGROUND_ALLOW_ALL_ORIGINS first; if truthy returns "*".
      # Otherwise returns HECKS_PLAYGROUND_CORS_ORIGIN if set, or nil if neither is set.
      #
      # @return [String, nil] the origin value to use, or nil to suppress the header
      def cors_origin_value
        return "*" if ENV["HECKS_PLAYGROUND_ALLOW_ALL_ORIGINS"].to_s.downcase == "true"

        origin = ENV["HECKS_PLAYGROUND_CORS_ORIGIN"]
