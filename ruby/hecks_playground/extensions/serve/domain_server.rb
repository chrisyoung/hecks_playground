...
HecksPlayground::Chapters.load_aggregates(
  HecksPlayground::Extensions::ServeChapter,
  base_dir: __dir__
)
HecksPlayground::Chapters.load_aggregates(
...
HecksPlayground::Chapters.load_aggregates(
  HecksPlayground::Extensions::AuthChapter,
  base_dir: File.expand_path("../auth", __dir__)
)

module HecksPlayground
  module HTTP
    # HecksPlayground::HTTP::DomainServer
    #
    # WEBrick-based REST server that serves a HecksPlayground domain over HTTP with CORS
    ...
    #
    #   hecks_playground serve pizzas_domain
    #   hecks_playground serve pizzas_domain --watch
    #
    class DomainServer
      include HecksTemplating::NamingHelpers
      include HecksPlayground::HTTP::CorsHeaders
      include CsrfHelpers
      include HecksPlayground::Auth::ScreenRoutes
      ...
      #
      # @param domain [HecksPlayground::Domain] the domain definition to serve
      ...
      # @return [void]
      def run
        puts "HecksPlayground serving #{@domain.name} on http://localhost:#{@port}"
        puts ""
      ...
      def reload!
        ...
        Kernel.load(source)
        new_domain = HecksPlayground.last_domain
        new_domain.source_path = source
        @lock.synchronize do
          @domain = new_domain
          rebuild_routes
        end
        puts "[hecks_playground] Domain reloaded at #{Time.now.strftime('%H:%M:%S')}"
      rescue => e
        warn "[hecks_playground] Reload failed: #{e.message}"
      end
      ...
      def handle(req, res)
        ...
        if req.path == "/events"
          res["Content-Type"] = "application/json"
          res.body = JSON.generate(app.events.map { |e| { type: HecksPlayground::Utils.const_short_name(e), occurred_at: e.occurred_at.iso8601 } })
          return
        end

        if req.path == "/_openapi"
          res["Content-Type"] = "application/json"
          res.body = JSON.generate(HecksPlayground::HTTP::OpenapiGenerator.new(domain).generate)
          return
        end

        if req.path == "/_schema"
          res["Content-Type"] = "application/json"
          res.body = JSON.generate(HecksPlayground::HTTP::JsonSchemaGenerator.new(domain).generate)
          return
        end
      ...
      def restore_actor_from_session(req)
        ...
        HecksPlayground.actor = actor if actor
      end

      # Start a WebSocket server for real-time event streaming.
      #
      # Requires the +hecks_playground_sockets+ gem and starts an async WebSocket
      ...
      def start_websocket_server
        require "hecks_playground_sockets"
        handler = HecksSockets::WebsocketHandler.new(@domain, @app)
        bus = @app.event_bus
        ws = HecksSockets::WebsocketServer.new(handler, bus, gate: @live_port)
        ws.start_async
      rescue LoadError
        warn "[hecks_playground] hecks_playground_sockets gem not found — skipping WebSocket server"
      end
      ...
      def start_watcher
        ...
        unless watch_dir
          warn "[hecks_playground] No source_path on domain — cannot watch for changes"
          return
        end
      ...
      def boot_domain
        ...
        unless Object.const_defined?(mod_name)
          tmpdir = Dir.mktmpdir("hecks_playground_serve")
          gem_path = HecksPlayground.build(@domain, output_dir: tmpdir)
      ...
      #
      # Delegates to {HecksPlayground::HTTP::CorsHeaders#apply_cors_origin} which
      # checks HECKS_PLAYGROUND_ALLOW_ALL_ORIGINS and HECKS_PLAYGROUND_CORS_ORIGIN env vars.
      #
