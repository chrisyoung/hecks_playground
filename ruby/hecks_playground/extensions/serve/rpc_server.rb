...
module HecksPlayground
  module HTTP
    # HecksPlayground::HTTP::RpcServer
    #
    # WEBrick-based JSON-RPC 2.0 server for a HecksPlayground domain. Provides a single
    ...
    #
    #   hecks_playground serve pizzas_domain --rpc
    #
    #   # JSON-RPC request:
    ...
    class RpcServer
      ...
      def initialize(domain, port: 9292)
        ...
        @methods = {}
        boot_domain
        @whitelist = HecksPlayground::Conventions::DispatchContract.build_whitelist(domain)
        register_methods
      end
      ...
      # @return [void]
      def run
        puts "HecksPlayground RPC serving #{@domain.name} on http://localhost:#{@port}"
        puts ""
        puts "Methods:"
      ...
      def boot_domain
        ...
        unless Object.const_defined?(mod_name)
          tmpdir = Dir.mktmpdir("hecks_playground_rpc")
      ...
      #
      # @param agg [HecksPlayground::BluebookModel::Structure::Aggregate] the aggregate definition
      ...
      def register_commands(agg, klass)
        ...
        agg.commands.each do |cmd|
          ...
          HecksPlayground::Conventions::DispatchContract.validate!(@whitelist, agg.name, method_name)
      ...
      def register_queries(agg, klass)
        ...
        agg.queries.each do |query|
          ...
          HecksPlayground::Conventions::DispatchContract.validate!(@whitelist, agg.name, qn.to_sym)
      ...
      # Serialize a domain object into a plain Hash using HecksPlayground::Utils.
      #
      # @param obj [Object] the domain object to serialize
      # @return [Hash{String => Object}] serialized attribute hash
      def serialize(obj)
        HecksPlayground::Utils.serialize_object(obj)
      end
    end
  end
end
