...
module HecksPlayground
  module HTTP
    class MultiDomainServer
      module UIRoutes
        # HecksPlayground::HTTP::MultiDomainServer::UIRoutes::EventRoutes
        #
        ...
        module EventRoutes
          ...
          def serve_events_json(res)
            ...
            res.body = JSON.generate(evts.map { |e|
              { type: HecksPlayground::Utils.const_short_name(e),
                occurred_at: (e.occurred_at.iso8601 rescue nil) }
            })
          end

          def serve_events(req, res)
            require "hecks_playground/extensions/web_explorer/paginator"
            buses = @entries.map { |e| e[:runtime].event_bus }
            ei = HecksPlayground::WebExplorer::EventIntrospector.new(buses)
            tf, af = req.query["type"].to_s, req.query["aggregate"].to_s
            all = ei.all_entries(type_filter: tf, aggregate_filter: af)
            pager = HecksPlayground::WebExplorer::Paginator.new(all, page: (req.query["page"] || 1).to_i)
