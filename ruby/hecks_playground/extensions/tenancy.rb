# HecksTenancy
#
# Multi-tenancy extension for HecksPlayground domains. Wraps repositories with
# tenant-scoped proxies so each tenant sees isolated data. The tenant
# identity is set globally via +HecksPlayground.tenant = "acme"+ and all subsequent
...
# tenancy configured.
#
# Future gem: hecks_playground_tenancy
#
#   # Gemfile
#   gem "cats_domain"
...
# @param domain [HecksPlayground::Domain] the parsed domain definition; must respond
#   to +tenancy+ returning a truthy value for the extension to activate
# @param runtime [HecksPlayground::Runtime] the runtime instance whose adapters will be
#   wrapped with tenant-scoped proxies
HecksPlayground.register_extension(:tenancy) do |domain_mod, domain, runtime|
  tenancy = domain.respond_to?(:tenancy) && domain.tenancy
  tenancy ||= HecksPlayground.last_hecksagon&.tenancy
  next unless tenancy
