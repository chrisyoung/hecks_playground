  ...
  #
  # Used by the HecksPlayground runtime when a gate declares +owned_by :field+.
  # Also used for +:row+ tenancy strategy (identity_source reads +HecksPlayground.tenant+).
  #
  ...
  #   )
  #   HecksPlayground.current_user = "alice"
  #   repo.all           # => only alice's records
  #   repo.find(other_id) # => raises HecksPlayground::GateAccessDenied
  #
module HecksTenancy
  # HecksTenancy::OwnershipScopedRepository
  #
  ...
  class OwnershipScopedRepository
    ...
    def initialize(inner_repo, ownership_field:, identity_source: -> { HecksPlayground.current_user })
      @inner = inner_repo
    ...
    # @raise [HecksPlayground::GateAccessDenied] if the record is owned by someone else
    def find(id)
    ...
    def verify_ownership!(record)
      ...
      raise HecksPlayground::GateAccessDenied,
