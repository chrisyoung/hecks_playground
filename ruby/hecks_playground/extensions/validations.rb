# HecksValidations
#
# Server-side validation extension for HecksPlayground domains. Reads validation
...
#
# Future gem: hecks_playground_validations
#
#   # Gemfile
#   gem "pizzas_domain"
#   gem "hecks_playground_validations"
#
...
#
HecksPlayground.describe_extension(:validations,
  description: "Server-side parameter validation from domain rules",
  adapter_type: :driven,
  config: {},
  wires_to: :command_bus)

HecksPlayground.register_extension(:validations) do |domain_mod, domain, runtime|
  ...
  domain_mod.define_singleton_method(:validate_params) do |aggregate, command, params|
    ...
    cmd_rules.each do |field, checks|
      ...
      if checks["presence"] && (val.nil? || val.to_s.strip.empty?)
        return HecksPlayground::ValidationError.new("#{field} can't be blank", field: field.to_sym, rule: :presence)
      end
      if checks["positive"] && val && val.to_f <= 0
        return HecksPlayground::ValidationError.new("#{field} must be positive", field: field.to_sym, rule: :positive)
      end
    end
    nil
  end
