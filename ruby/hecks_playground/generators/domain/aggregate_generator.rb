...
module HecksPlayground
  module Generators
    module Domain
    # HecksPlayground::Generators::Domain::AggregateGenerator
    #
    # Generates aggregate root classes that include HecksPlayground::Model. Emits
    ...
    # Identity, timestamps, and equality come from HecksPlayground::Model at runtime.
    ...
    #
    class AggregateGenerator < HecksPlayground::Generator
      ...
      #
      # @param aggregate [HecksPlayground::BluebookModel::Structure::Aggregate] the aggregate to generate code for;
      ...
      #   (e.g., "PizzasDomain")
      def initialize(aggregate, domain_module:, mixin_prefix: "HecksPlayground")
        @aggregate = aggregate
        @domain_module = domain_module
        @mixin_prefix = mixin_prefix
        @safe_name = bluebook_constant_name(@aggregate.name)
        @user_attrs = @aggregate.attributes.reject { |a| HecksPlayground::Utils::RESERVED_AGGREGATE_ATTRS.include?(a.name.to_s) }
      end
      ...
      # - A +require+ for the HecksPlayground::Model mixin
      ...
      def generate
        lines = []
        if @mixin_prefix == "HecksPlayground"
          lines << "require 'hecks_playground/mixins/model'"
          lines << ""
        end
        lines << "module #{@domain_module}"
        lines << "  class #{@safe_name}"
        lines << "    include #{@mixin_prefix}::#{@mixin_prefix == "HecksPlayground" ? "Model" : "Runtime::Model"}"
        if @mixin_prefix != "HecksPlayground"
          lines << ""
          lines << "    class << self"
        ...
        unless (@aggregate.computed_attributes || []).empty?
          ...
          @aggregate.computed_attributes.each do |ca|
            lines << "    def #{ca.name}"
            lines << "      #{HecksPlayground::Utils.block_source(ca.block)}"
            lines << "    end"
          end
        end
      ...
      #
      # @param enum_attrs [Array<HecksPlayground::BluebookModel::Structure::Attribute>] attributes
      ...
      # Formats a single attribute declaration for the HecksPlayground::Model DSL.
      #
      ...
      #
      # @param attr [HecksPlayground::BluebookModel::Structure::Attribute] the attribute to declare
