HecksPlayground::Chapters.load_aggregates(
  HecksPlayground::Bluebook::GeneratorInternalsParagraph,
...
module HecksPlayground
  module Generators
    module Domain
    # HecksPlayground::Generators::Domain::CommandGenerator
    #
    ...
    # safe attribute names via **kwargs. The HecksPlayground::Command mixin (included
    ...
    # - +include HecksPlayground::Command+ for event emission and handler wiring
    ...
    #   gen.generate
    #
    class CommandGenerator < HecksPlayground::Generator
      include InjectionHelpers

      # Initializes the command generator.
      #
      # @param command [HecksPlayground::BluebookModel::Behavior::Command] the command model object;
      ...
      # @param aggregate [HecksPlayground::BluebookModel::Structure::Aggregate, nil] the aggregate model object,
      ...
      def initialize(command, domain_module:, aggregate_name:, aggregate: nil, event: nil, mixin_prefix: "HecksPlayground")
        @command = command
        ...
        @has_keyword_attrs = @command.attributes.any? { |attr| HecksPlayground::Utils.ruby_keyword?(attr.name) }
      ...
      def generate
        ...
        lines << "        include #{@mixin_prefix}::#{@mixin_prefix == "HecksPlayground" ? "Command" : "Runtime::Command"}"
        if @event
      ...
      def given_then_lines
        ...
        @command.givens.each do |g|
          ...
          lines << "          raise HecksPlayground::PreconditionError, #{msg.inspect} unless agg.instance_eval { #{g.expression} }"
        end
      ...
      def custom_call_lines
        source = HecksPlayground::Utils.block_source(@command.call_body)
        lines = ["        def call"]
      ...
      # Raises +HecksPlayground::Error+ if the entity is not found.
      #
      ...
      def find_self_ref(agg_snake)
        (@command.references || []).find do |ref|
          HecksPlayground::Utils.underscore(ref.type) == agg_snake
        end
      end
    end
    end
