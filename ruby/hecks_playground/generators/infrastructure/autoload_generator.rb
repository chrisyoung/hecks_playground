...
module HecksPlayground
  module Generators
    ...
    # HecksPlayground::Generators::Infrastructure::AutoloadGenerator
    #
    ...
    #
    class AutoloadGenerator < HecksPlayground::Generator
      # Creates a new AutoloadGenerator for a domain.
      #
      # @param domain [HecksPlayground::BluebookModel::Structure::Domain] the parsed domain IR
      ...
      #   and wires a +HecksPlayground::Runtime+ when the gem is installed alongside HecksPlayground
      ...
      def generate_entry_point
        ...
        lines << "end"
        lines << ""
        lines << "# Auto-boot: wire a Runtime when hecks_playground is available and gem is installed."
        lines << "# Add extension gems to your Gemfile to auto-wire:"
        lines << "#   hecks_playground_sqlite  → SQLite persistence"
        lines << "#   hecks_playground_postgres → PostgreSQL persistence"
        lines << "#   hecks_playground_mysql   → MySQL persistence"
        lines << "#   hecks_playground_serve   → HTTP/JSON-RPC server"
        lines << "#   hecks_playground_ai      → MCP server for AI agents"
        lines << "# Remove a gem to unwire that extension. No code changes needed."
        lines << "if defined?(HecksPlayground) && Gem.loaded_specs[\"#{gem_name}\"]"
        lines << "  _hecks_playground_dir = File.join(Gem.loaded_specs[\"#{gem_name}\"].full_gem_path, \"hecks_playground\")"
        lines << "  _hecks_playground_domain_files = Dir[File.join(_hecks_playground_dir, \"*.bluebook\")] if File.directory?(_hecks_playground_dir)"
        lines << "  if _hecks_playground_domain_files&.any?"
        lines << "    _hecks_playground_domain_files.each { |f| Kernel.load(f) }"
        lines << "    #{mod}.instance_variable_set(:@_hecks_playground_domain, HecksPlayground.last_domain)"
        lines << "  end"
        lines << ""
        lines << "  #{mod}.define_singleton_method(:boot) do |**opts, &block|"
        lines << "    domain = instance_variable_get(:@_hecks_playground_domain)"
        lines << "    return unless domain"
        lines << "    @runtime = HecksPlayground.load(domain, **opts, &block)"
        lines << "    HecksPlayground.extension_registry.each { |_name, hook| hook.call(self, domain, @runtime) }"
        lines << "    @runtime"
        lines << "  end"
        lines << ""
        lines << "  #{mod}.define_singleton_method(:runtime) { @runtime }"
        lines << ""
        lines << "  #{mod}.boot unless ENV[\"HECKS_PLAYGROUND_SKIP_BOOT\"]"
        lines << "end"
        lines.join("\n") + "\n"
      end
      ...
      # +const_missing+ in +HecksPlayground::Model+.
      #
      # @param aggregate [HecksPlayground::BluebookModel::Structure::Aggregate] the aggregate
      ...
      #
      # @param aggregate [HecksPlayground::BluebookModel::Structure::Aggregate]
