  ...
  module Generators
    module Infrastructure
      class DomainGemGenerator < HecksPlayground::Generator
        # HecksPlayground::Generators::Infrastructure::DomainGemGenerator::FileWriter
        #
        ...
        module FileWriter
          ...
          def generate_gemspec(root, gem_name, mod)
            ...
            write_file(root, "#{gem_name}.gemspec", <<~RUBY)
              Gem::Specification.new do |s|
                ...
                s.authors = ["Generated by HecksPlayground"]; s.summary = "#{@domain.name} domain model"
                s.description = "Pure domain gem generated by HecksPlayground"; s.license = "MIT"
                s.files = Dir["lib/**/*", "docs/**/*", "*Bluebook", "verbs.txt"]; s.require_paths = ["lib"]
                s.required_ruby_version = ">= 3.0"
                s.add_dependency "hecks_playground"
          ...
          def write_command_file(root, relative_path, new_content, command)
            full_path = HecksPlayground::Utils.safe_path!(root, relative_path)
            if !command.call_body && File.exist?(full_path)
              existing_source = File.read(full_path)
              existing_call = HecksPlayground::Utils.extract_call_method(existing_source)
              generated_call = HecksPlayground::Utils.extract_call_method(new_content)
          ...
          def write_file(root, relative_path, content)
            path = HecksPlayground::Utils.safe_path!(root, relative_path)
