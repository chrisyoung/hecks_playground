# HecksPlayground::Generators::Infrastructure::SpecGenerator::PortSpec
#
...
module HecksPlayground
  module Generators
    module Infrastructure
      class SpecGenerator < HecksPlayground::Generator
        module PortSpec
          include HecksTemplating::NamingHelpers
          # Generates an RSpec spec for a port on an aggregate.
          #
          # @param port_name [Symbol] the port name (e.g., :admin, :guest)
          # @param port_def [HecksPlayground::BluebookModel::Structure::GateDefinition]
          ...
          def generate_port_spec(port_name, port_def, aggregate)
            ...
            lines << "  before { @app = HecksPlayground.load(domain, gate: :#{port_name}, force: true) }"
            lines << ""
            ...
            denied.each do |method|
              ...
              lines << "    expect { #{safe_agg}.#{method}#{method == :find ? '("test")' : ''} }.to raise_error(HecksPlayground::GateAccessDenied)"
              lines << "  end"
              lines << ""
            end
            ...
            aggregate.commands.each do |cmd|
              ...
              unless allowed.include?(cmd_method.to_sym)
                ...
                lines << "    expect { #{safe_agg}.#{cmd_method}(#{example_args(cmd)}) }.to raise_error(HecksPlayground::GateAccessDenied)"
                lines << "  end"
                lines << ""
              end
            end
          ...
          def find_port_create_cmd(aggregate)
            aggregate.commands.find do |cmd|
              HecksPlayground::Conventions::CommandContract.find_self_ref(cmd.attributes, aggregate.name).nil?
            end
          end

          def derive_port_method(cmd, agg)
            HecksPlayground::Conventions::CommandContract.method_name(cmd.name, agg.name).to_s
          end
        end
      end
    end
