HecksPlayground::Chapters.load_aggregates(
  HecksPlayground::Runtime::Mixins,
  base_dir: File.expand_path("command", __dir__)
)
  # HecksPlayground::Command
  #
...
module HecksPlayground
  ...
  #
  module Command
    # Hook called when a class includes +HecksPlayground::Command+. Extends the class
    ...
    # are wired during boot by the HecksPlayground runtime.
    module ClassMethods
      ...
      def event_class
        agg_module = HecksPlayground::Conventions::Names.aggregate_module_from_command(name)
      ...
      # @raise [HecksPlayground::GuardRejected] if the guard policy rejects the command
      # @raise [HecksPlayground::PreconditionError] if any precondition fails
      # @raise [HecksPlayground::PostconditionError] if any postcondition fails
      def call(**attrs)
        cmd = new(**attrs)
    ...
    # @yieldreturn [HecksPlayground::Command] the next command in the chain
    # @return [HecksPlayground::Command] the next command, or self if an error occurred
    #
    # @example
    ...
    #
    # @return [Array<HecksPlayground::Command>] all commands executed in the chain
    def steps = @chain_steps || [self]

    # Returns the last command in the chain.
    #
    # @return [HecksPlayground::Command]
    def last = steps.last
    ...
    def method_missing(name, *args, **kwargs, &block)
      agg = @aggregate
      if agg && !agg.equal?(self) && !agg.is_a?(HecksPlayground::Command) && agg.respond_to?(name)
    ...
    def respond_to_missing?(name, include_private = false)
      agg = @aggregate
      return super if agg.nil? || agg.equal?(self) || agg.is_a?(HecksPlayground::Command)
    ...
    #
    # @return [Object] the repository instance (typically a HecksPlayground memory or SQL adapter)
    def repository
    ...
    def enforce_role!
      ...
      current = HecksPlayground.respond_to?(:current_role) ? HecksPlayground.current_role : nil
      ...
      unless role_names.include?(current_name)
        ...
        raise HecksPlayground::Unauthorized, "#{cmd_name} requires role: #{role_names.join(" or ")}. Current: #{current_name.empty? ? "none" : current_name}"
      end
    end
    ...
    def run_guard
      ...
      agg_module = HecksPlayground::Conventions::Names.aggregate_module_from_command(self.class.name)
      policy_class = Object.const_get("#{agg_module}::Policies::#{policy_name}")
      result = policy_class.new.call(self)
      unless result
        raise HecksPlayground::GuardRejected, "Guard #{policy_name} rejected #{self.class.name.split('::').last}"
      end
    end
