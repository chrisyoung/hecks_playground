# HecksPlayground::Command::LifecycleSteps
#
    ...
    module LifecycleSteps
          ...
          if existing
            ...
            if current
              ...
              if transition && transition[:from] && !Array(transition[:from]).include?(current)
                raise HecksPlayground::TransitionError.new(
      ...
      GivenStep = ->(cmd) {
        if cmd.class.respond_to?(:givens_ir) && cmd.class.givens_ir&.any?
          require "hecks_playground/runtime/hecksal_interpreter"
      ...
      def self.build_aggregate_for(cmd)
        ...
        attrs = {}
        if agg_class.respond_to?(:hecks_playground_attributes)
          agg_class.hecks_playground_attributes.each do |attr|
