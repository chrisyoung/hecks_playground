  ...
  # process asynchronously. Swap via +HecksPlayground.boot { queue :sidekiq }+.
  #
  ...
  # == Usage
  #
  #   handle = HecksPlayground.queue.enqueue("RegisterModel", name: "GPT-5")
...
module HecksPlayground
  module Queue
    ...
    class MemoryQueue
      ...
      # @raise [HecksPlayground::Error] if no command resolver is configured
      def execute(command_name, attrs)
        if @command_resolver
          @command_resolver.call(command_name, attrs)
        else
          raise HecksPlayground::Error, "No command resolver configured for queue"
        end
      end
    end
  end
