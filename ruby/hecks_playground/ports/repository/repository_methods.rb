module HecksPlayground
  module Persistence
    # HecksPlayground::Persistence::RepositoryMethods
    #
    # Adds CRUD persistence methods to aggregate classes. Bound automatically
    # by +Persistence.bind+ during application boot. Uses +hecks_playground_attributes+
    ...
    module RepositoryMethods
      ...
      def self.bind(klass, repo)
        klass.instance_variable_set(:@__hecks_playground_repo__, repo)
      ...
      #
      # Prefers +hecks_playground_attributes+ metadata if available (set by the code generator),
      ...
      def self.attr_names(klass)
        if klass.respond_to?(:hecks_playground_attributes)
          klass.hecks_playground_attributes.map(&:name)
        else
