# HecksPlayground::Boot
#
# Loads all .bluebook domains from a hecks_playground/ directory, validates, builds,
# and wires Runtimes with shared event bus and extensions.
#
#   runtimes = HecksPlayground.boot(__dir__)
#
module HecksPlayground
  # HecksPlayground::Boot
  #
  # Boots all domains from the hecks_playground/ directory in a project.
  #
  module Boot
    include HecksTemplating::NamingHelpers

    # @return [HecksPlayground::EventBus, nil] the shared bus from the last boot
    attr_reader :shared_event_bus

    # Boot from a directory (hecks_playground/*.bluebook) or a pre-built domain.
    #
    # @param dir [String] project directory containing hecks_playground/ subdirectory
    ...
    # @return [HecksPlayground::Runtime, Array<HecksPlayground::Runtime>]
    def boot(dir = Dir.pwd, domain: nil, root: nil, source_dir: nil)
      ...
      if domain
        ...
        # Boot from hecks_playground/*.bluebook files
        hecks_playground_dir = File.join(dir, "hecks_playground")
        raise HecksPlayground::BluebookLoadError, "No hecks_playground/ directory in #{dir}" unless File.directory?(hecks_playground_dir)

        bluebooks = find_domain_files(hecks_playground_dir)
        raise HecksPlayground::BluebookLoadError, "No .bluebook files in #{hecks_playground_dir}" if bluebooks.empty?

        find_hecksagon_files(hecks_playground_dir).each { |f| Hecksagon::Loader.load(f) }
        find_world_files(hecks_playground_dir).each { |f| Hecksagon::Loader.load_world(f) }

        domains = bluebooks.map do |path|
          HecksPlayground.instance_variable_set(:@_inferred_bluebook_name,
    ...
    def fire_extensions(mod, domain, runtime)
      ...
      eligible = HecksPlayground.extension_registry.select do |name, hook|
    ...
    def partition_by_adapter_type(extensions)
      ...
      extensions.each do |name, hook|
        meta = HecksPlayground.extension_meta[name]
    ...
    def boot_domains(domains, root: nil)
      ...
      HecksPlayground::MultiDomain::Validator.validate_no_cross_domain_references(domains)
      attach_context_map(domains)
      domains.each { |d| load_bluebook(d) }

      shared_bus = EventBus.new
      @shared_event_bus = shared_bus
      directionality = HecksPlayground::MultiDomain::Directionality.build(domains)
      primary = nil
    ...
    def wire_persistence(runtimes)
      runtimes.each do |rt|
        ...
        hook = HecksPlayground.extension_registry[hecksagon.persistence[:type]]
        next unless hook
    ...
    # dispatches through HecksPlayground::Runtime::ShellDispatcher.
    ...
    # dispatches through HecksPlayground::Runtime::LlmDispatcher. Mirrors
    ...
    def attach_context_map(domains)
      hecksagon = HecksPlayground.last_hecksagon
