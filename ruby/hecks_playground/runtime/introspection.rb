...
module HecksPlayground
  ...
  module Introspection
      ...
      # @param agg [HecksPlayground::BluebookModel::Structure::Aggregate] the aggregate IR definition
      # @return [void]
      def self.bind(klass, agg)
        klass.instance_variable_set(:@__hecks_playground_agg_def__, agg)
      ...
      module ClassMethods
        ...
        #
        # @return [HecksPlayground::BluebookModel::Structure::Aggregate] the aggregate IR definition
        def domain_def
          instance_variable_get(:@__hecks_playground_agg_def__)
        end
        ...
        # @return [nil]
        def describe
          puts HecksPlayground::AggregateDescriber.describe_lines(domain_def).join("\n")
          nil
        end
        ...
        def domain_commands
          ...
          agg.commands.each_with_index.map do |cmd, i|
            event = agg.events[i]
            attrs = cmd.attributes.map { |a| "#{a.name}: #{HecksPlayground::Utils.type_label(a)}" }.join(", ")
        ...
        def domain_entities
          domain_def.entities.map do |ent|
            attrs = ent.attributes.map { |a| "#{a.name}: #{HecksPlayground::Utils.type_label(a)}" }.join(", ")
            "#{ent.name} (#{attrs})"
          end
        end
        ...
        def domain_value_objects
          domain_def.value_objects.map do |vo|
            attrs = vo.attributes.map { |a| "#{a.name}: #{HecksPlayground::Utils.type_label(a)}" }.join(", ")
            "#{vo.name} (#{attrs})"
          end
        end
        ...
        # @return [void]
        def glossary
          HecksPlayground::DomainGlossary.new(nil).print_for(domain_def)
        end
      end
  end
end
