require "hecks_playground/runtime/llm_providers"

module HecksPlayground
  class Runtime

    # HecksPlayground::Runtime::LlmDispatcher
    #
    ...
    #                  → re-raise as HecksPlayground::LlmInvocationFailed
    #
    ...
    #
    # Mirrors HecksPlayground::Runtime::ShellDispatcher in surface so callers can
    ...
    #   )
    #   provider = HecksPlayground::Runtime::LlmProviders::TestProvider.new(
    ...
    module LlmDispatcher
      ...
      # @raise [HecksPlayground::LlmInvocationFailed] on provider transport failure
      ...
      def resolve_provider(backend, providers)
        ...
        if providers.nil? && backend == :test
          return HecksPlayground::Runtime::LlmProviders::TestProvider.new
        end
        Skipped.new(
      ...
      def invoke_with_recording(adapter, provider, backend, prompt, stream,
        ...
        result
      rescue HecksPlayground::LlmInvocationFailed => e
        record_failure(kind, breaker, logger, e)
        raise
      rescue HecksPlayground::LlmAdapterError => e
        record_failure(kind, breaker, logger, e)
