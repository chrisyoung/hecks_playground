...
module HecksPlayground
  class Runtime
    module LlmProviders
      ...
      # HecksPlayground::Runtime::LlmProviders::ClaudeProvider
      #
      ...
      # raises HecksPlayground::LlmInvocationSkipped(reason: "provider_unavailable").
      #
      ...
      class ClaudeProvider
        ...
        def invoke_cli(prompt, model, max_tokens, stream)
          ...
          unless status.respond_to?(:success?) ? status.success? : status.to_i.zero?
            raise HecksPlayground::LlmAdapterError,
          ...
          @degraded = true
          @degraded_reason = e.message
          raise HecksPlayground::LlmAdapterError, "claude CLI not found: #{e.message}"
        end
        ...
        def invoke_api_unary(http, req, model)
          ...
          raise HecksPlayground::LlmAdapterError, "claude API #{res.code}: #{res.body}" \
            unless res.code == "200"
        ...
        def invoke_api_stream(http, req, model)
          ...
          http.request(req) do |res|
            raise HecksPlayground::LlmAdapterError, "claude API #{res.code}" \
              unless res.code == "200"
            buf = +""
