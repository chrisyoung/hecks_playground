...
module HecksPlayground
  class Runtime
    module LlmProviders
      ...
      # HecksPlayground::Runtime::LlmProviders::OllamaProvider
      #
      ...
      # HecksPlayground::LlmInvocationSkipped(reason: "provider_unavailable").
      #
      ...
      class OllamaProvider
        ...
        # @raise [HecksPlayground::LlmAdapterError] on transport failure
        # @raise [HecksPlayground::LlmInvocationSkipped] when degraded
        def invoke(prompt:, model:, max_tokens:, stream: false, &block)
          if @degraded
            raise HecksPlayground::LlmInvocationSkipped.new(reason: "provider_unavailable")
          end
        ...
        rescue HecksPlayground::LlmAdapterError, HecksPlayground::LlmInvocationSkipped
          raise
        rescue StandardError => e
          raise HecksPlayground::LlmAdapterError, "ollama transport error: #{e.message}"
        end
        ...
        def invoke_unary(http, req, model)
          ...
          raise HecksPlayground::LlmAdapterError, "ollama #{res.code}: #{res.body}" \
            unless res.code == "200"
        ...
        def invoke_stream(http, req, model)
          ...
          http.request(req) do |res|
            raise HecksPlayground::LlmAdapterError, "ollama #{res.code}" \
              unless res.code == "200"
            buf = +""
