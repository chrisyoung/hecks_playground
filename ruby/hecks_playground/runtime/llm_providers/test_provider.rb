...
module HecksPlayground
  class Runtime
    module LlmProviders
      ...
      # HecksPlayground::Runtime::LlmProviders::TestProvider
      #
      ...
      #   - strict (env HECKS_PLAYGROUND_LLM_FIXTURE_STRICT=1): unknown hashes
      #     raise HecksPlayground::LlmInvocationFailed(reason: "fixture_missing").
      #     Used by CI so a missing fixture fails fast.
      #
      # Capture/replay seam:
      #   HECKS_PLAYGROUND_LLM_REPLAY=path  — load fixtures from a YAML file at
      #                            construction (one entry per
      #                            prompt_sha256). Step 4 only wires
      #                            the seam ; the file format is owned
      #                            by a follow-up step.
      #   HECKS_PLAYGROUND_LLM_CAPTURE=path — placeholder ; this step does not yet
      ...
      class TestProvider
        ...
        # @param capture_path [String, nil] from HECKS_PLAYGROUND_LLM_CAPTURE (seam only)
        def initialize(fixtures: {}, strict: nil, capture_path: nil, env: ENV)
          @fixtures = stringify_fixture_keys(fixtures)
          @strict = strict.nil? ? env["HECKS_PLAYGROUND_LLM_FIXTURE_STRICT"] == "1" : !!strict
          @capture_path = capture_path || env["HECKS_PLAYGROUND_LLM_CAPTURE"]
        end
        ...
        # @return [Boolean] true if HECKS_PLAYGROUND_LLM_FIXTURE_STRICT=1 (or strict: true was passed)
        def strict?
          @strict
        end
        ...
        # @return [HecksPlayground::Runtime::LlmProviders::Result]
        # @raise [HecksPlayground::LlmInvocationFailed] when strict mode and no fixture
        def invoke(prompt:, model: nil, max_tokens: nil, stream: false)
          ...
          response_text =
            if @fixtures.key?(digest)
              ...
              raise HecksPlayground::LlmInvocationFailed.new(
