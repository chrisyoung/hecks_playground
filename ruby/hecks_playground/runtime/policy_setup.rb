...
module HecksPlayground
  class Runtime
      # HecksPlayground::Runtime::PolicySetup
      #
      ...
      module PolicySetup
        ...
        def execute_policy(policy, policy_key, event)
          ...
          begin
            ...
            warn "[HecksPlayground] Policy #{policy.name} failed: #{e.message}"
          ensure
        ...
        def reentrant?(policy_key, name)
          if @policies_in_flight.include?(policy_key)
            warn "[HecksPlayground] Skipping re-entrant policy #{name} (already in-flight)"
            true
          end
        end
