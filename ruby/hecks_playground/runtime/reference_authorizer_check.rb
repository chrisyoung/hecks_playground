# HecksPlayground::Runtime::ReferenceCoverageCheck
#
...
module HecksPlayground
  class Runtime
    ...
    module ReferenceCoverageCheck
      ...
      #
      # Raises +HecksPlayground::ConfigurationError+ if authorization-required references
      # exist but no reference_authorizer is registered on the command class.
      #
      # @return [void]
      # @raise [HecksPlayground::ConfigurationError] when auth-required references have no authorizer
      ...
      def resolve_command_class(agg, cmd)
        HecksPlayground::Conventions::Names.resolve_command_const(@mod, agg.name, cmd.name)
      rescue NameError
        nil
      end
    end
