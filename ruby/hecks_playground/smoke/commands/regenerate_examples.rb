HecksPlayground::CLI.register_command(:regenerate_examples, "Regenerate all example outputs from the pizzas domain") do
  root = File.expand_path("../..", __dir__)
  pizzas = File.join(root, "examples", "pizzas")

  Dir.chdir(pizzas) do
    say "Loading pizzas domain...", :green
    bluebook = Dir["hecks_playground/*.bluebook"].first || Dir["*.bluebook"].first
    ...
    HecksPlayground.build(domain, output_dir: ".")
    ...
    HecksPlayground.target_registry[:static].call(domain, output_dir: ".", smoke_test: false)

    say "Building Go binary...", :green
    FileUtils.rm_rf("pizzas_static_go")
    require "go_hecks_playground"
    HecksPlayground.target_registry[:go].call(domain, output_dir: ".", smoke_test: false)

    say "Building Rails app...", :green
    FileUtils.rm_rf(File.join(root, "examples", "pizzas_rails"))
    require "hecks_playground/generators/rails_generator"
