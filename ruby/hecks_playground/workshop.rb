...
require "hecks_playground/chapters/workshop"
HecksPlayground::Chapters.load_chapter(
  HecksPlayground::Workshop,
  base_dir: __dir__
)
...
module HecksPlayground
  ...
  #
  #   workshop = HecksPlayground.workshop("Pizzas")
  ...
  class Workshop
    ...
    #
    # Loads the domain into memory, then attempts to require the active_hecks_playground gem
    # and activate it. Prints a warning if the gem is not installed.
    #
    # @return [Module] the loaded domain module, or nil if active_hecks_playground is unavailable
    def active_hecks_playground!
      @active_hecks_playground = true
      domain = to_domain
      mod = HecksPlayground.load_domain(domain, force: true, skip_validation: true)
      begin
        require "active_hecks_playground"
      rescue LoadError
        puts "active_hecks_playground gem not installed. Add it to your Gemfile."
        return mod
      end
    ...
    #
    # @return [Boolean] true if +active_hecks_playground!+ has been called
    def active_hecks_playground?
      @active_hecks_playground
    end
