...
module HecksPlayground
  class Workshop
    # HecksPlayground::Workshop::Presenter
    #
    ...
    module Presenter
        ...
        domain.aggregates.each do |agg|
          ...
          unless agg.value_objects.empty?
            agg.value_objects.each do |vo|
              vo_attrs = vo.attributes.map { |a| "#{a.name}: #{HecksPlayground::Utils.type_label(a)}" }.join(", ")
          ...
          unless agg.entities.empty?
            agg.entities.each do |ent|
              ent_attrs = ent.attributes.map { |a| "#{a.name}: #{HecksPlayground::Utils.type_label(a)}" }.join(", ")
      ...
      #
      # @return [String] e.g. '#<HecksPlayground::Workshop "Pizzas" [play] (3 aggregates)>'
      def inspect
        mode_label = play? ? "play" : "sketch"
        "#<HecksPlayground::Workshop \"#{@name}\" [#{mode_label}] (#{@aggregate_builders.size} aggregates)>"
      end
    end
  end
end
