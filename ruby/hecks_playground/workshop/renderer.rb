# HecksPlayground::Workshop::Renderer
#
  ...
  class Workshop
    class Renderer
      ...
      def render_attribute(attr, depth)
        "#{indent(depth)}#{attr.name}: #{HecksPlayground::Utils.type_label(attr)}"
      end
      ...
      def render_event(event, depth)
        attrs = event.attributes.map { |a| "#{a.name}: #{HecksPlayground::Utils.type_label(a)}" }.join(", ")
