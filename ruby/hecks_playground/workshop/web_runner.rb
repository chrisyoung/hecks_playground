...
HecksPlayground::Chapters.load_aggregates(
  HecksPlayground::Workshop::WebComponentsParagraph,
  base_dir: File.expand_path("web_runner", __dir__)
)

module HecksPlayground
  class Workshop
    # HecksPlayground::Workshop::WebRunner
    #
    # Browser-based REPL for the HecksPlayground Workshop. Starts a WEBrick server
    ...
    class WebRunner
      ...
      def reload_domain!
        ...
        if @domain_paths
          ...
          name = workshop.name
          @runner.instance_variable_set(:@workshop, HecksPlayground::Workshop.new(name))
        end
        @serializer = StateSerializer.new(workshop, domain_groups: @domain_groups, domains: @loaded_domains)
        puts "Reset to #{@domain_paths ? 'loaded domains' : @domain_path ? 'loaded domain' : 'empty workshop'}"
      end

      BASE = "/hecks_playground_web_workbench"
      ...
      def run
        ...
        puts "HecksPlayground Web Workshop: http://localhost:#{@port}#{BASE}"
        server.start
      end
      ...
      def serve_command(req, res)
        reload_code! if ENV["HECKS_PLAYGROUND_DEV"]
      ...
      def load_domain_file(path)
        ...
        ws = HecksPlayground::Workshop.new(domain.name)
        domain.aggregates.each do |agg|
          ws.aggregate_builders[agg.name] =
            HecksPlayground::DSL::AggregateRebuilder.from_aggregate(agg)
      ...
      def load_multi_domain(paths, name)
        ws = HecksPlayground::Workshop.new(name || "Multi")
        paths.each do |path|
          Kernel.load(File.expand_path(path))
          domain = HecksPlayground.last_domain
          @loaded_domains << domain
