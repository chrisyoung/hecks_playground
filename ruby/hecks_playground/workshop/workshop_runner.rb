HecksPlayground::Chapters.load_aggregates(
  HecksPlayground::Workshop::RunnerSupportParagraph,
  base_dir: File.expand_path("workshop_runner", __dir__)
)

module HecksPlayground
  class Workshop
    # HecksPlayground::Workshop::WorkshopRunner
    #
    ...
    %i[
      aggregates remove add_verb active_hecks_playground!
    ...
    def inspect
      ...
      event ? "hecks_playground(#{name} #{mode}) [#{event}]" : "hecks_playground(#{name} #{mode})"
    end
    ...
    def tour
      @workshop = HecksPlayground.workshop("Tour")
      Tour.new(self).start
    end
    ...
    def last_event_label
      ...
      HecksPlayground::Utils.const_short_name(event)
    end

    def launch_irb
      history_file = File.join(Dir.home, ".hecks_playground_history")
      IRB.setup(nil)
    ...
    def setup_session
      if @name
        workshop = HecksPlayground.workshop(@name)
      ...
      if File.exist?("Bluebook")
        ...
        workshop = HecksPlayground.workshop(domain.name)
      ...
      workshop = HecksPlayground.workshop("MyDomain")
