# HecksPlayground::AI::Commands::Generate
#
...
#
#   hecks_playground generate "banking system with accounts, loans, and transfers"
#   hecks_playground generate "e-commerce platform" --output MyShopBluebook --dry-run
#
HecksPlayground::CLI.register_command(:generate, "Generate Bluebook DSL from a natural language description",
  args: ["DESCRIPTION"],
  options: {
  ...
  unless description && !description.strip.empty?
    say "Usage: hecks_playground generate \"describe your domain here\"", :red
    next
  end
  ...
  domain_json = HecksPlayground::AI::LlmClient.new(**client_opts).generate_domain(description)
  workshop    = HecksPlayground::AI::BluebookBuilder.new(domain_json).build
  dsl         = workshop.to_dsl
