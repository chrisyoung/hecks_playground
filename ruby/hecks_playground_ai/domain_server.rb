...
HecksPlayground::Chapters.load_aggregates(
  HecksPlayground::AI::DomainServerParagraph,
  base_dir: File.expand_path("domain_server", __dir__)
)

module HecksPlayground
  module MCP
    # HecksPlayground::MCP::DomainServer
    #
    # Generates an MCP server from a compiled HecksPlayground domain. Every command becomes
    ...
    #
    #   hecks_playground mcp --domain NAME
    #
    class DomainServer
      ...
      #
      # @param domain [HecksPlayground::BluebookModel::Structure::Domain] the domain model to serve
      def initialize(domain)
        @domain = domain
        @server = ::MCP::Server.new(
          name: "#{domain.name} Domain",
          version: HecksPlayground::VERSION
        )
        boot_and_register
      end
      ...
      # Boots a HecksPlayground::Runtime for the domain and binds repository methods
      ...
      #
      # @param attr [HecksPlayground::BluebookModel::Structure::Attribute] the attribute to inspect
      ...
      def serialize_aggregate(obj)
        attrs = HecksPlayground::Utils.object_attr_names(obj).map do |name|
