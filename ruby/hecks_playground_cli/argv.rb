...
#
#   argv = Argv.new
#   argv.register_from_bluebook("nursery/hecks_playground/compiler.bluebook")
  ...
  class CLI
    class Argv
      ...
      def register_from_bluebook(path)
        ...
        HecksPlayground::DSL::AggregateBuilder::VoTypeResolution.with_vo_constants do
          Kernel.load(File.expand_path(path))
        end
        domain = HecksPlayground.last_domain
        ...
        domain.aggregates.each do |agg|
          ...
          agg.commands.each do |cmd|
            ...
            handle(cli_name) do |inv|
              ...
              # Boot and dispatch
              rt = HecksPlayground.boot(File.dirname(File.dirname(bp)))
      ...
      def register_all_bluebooks(nursery_dir)
        Dir[File.join(nursery_dir, "hecks_playground", "*.bluebook")].sort.each do |path|
      ...
      def load_handler_file(path)
        ...
        elsif content.include?("HecksPlayground::CLI.handle")
      ...
      def route(inv)
        ...
        when "_help"
          print_grouped_help
        when "_version"
          puts "hecks_playground #{HecksPlayground::VERSION}"
        when "_command_help"
      ...
      def install_builtins!
        ...
        handle("inspect") do |inv|
          ...
          output = HecksPlayground::CLI::DomainInspector.new(domain).generate(aggregate: agg_filter)
          puts output
        end

        # list: show installed domain gems
        handle("list") do |inv|
          specs = ::Gem::Specification.select { |s| s.name.start_with?("hecks_playground_") || s.metadata["hecks_playground_domain"] }
          if specs.empty?
            puts "No HecksPlayground domains installed."
          else
            puts "\e[32mInstalled HecksPlayground domains:\e[0m"
      ...
      def print_grouped_help
        ...
        if top.any?
          top.sort.each do |name|
            entry = @commands[name]
            puts "  hecks_playground %-#{max}s  # %s" % [name, entry[:description]]
          end
          puts
        end
        ...
        puts "Run `hecks_playground help <command>` for details."
      end
      ...
      def print_error(msg)
        ...
        $stderr.puts
        $stderr.puts "Run `hecks_playground help` for available commands."
      end
      ...
      def load_domain(inv)
        ...
        $stderr.puts "\e[31mNo domain found. Pass a .bluebook file or run from a directory with hecks_playground/\e[0m"
        nil
      end
      ...
      # The file calls HecksPlayground::CLI.register_command(:name, desc) { |*args| ... }
      ...
      def load_legacy_handler(path)
        argv_instance = self
        handler_path = path
        original = HecksPlayground::CLI.method(:register_command) rescue nil
        HecksPlayground::CLI.define_singleton_method(:register_command) do |name, desc = nil, **opts, &block|
          next unless block
        ...
        begin
          ...
          $stderr.puts "  [argv] skip #{File.basename(path)}: #{e.message}" if ENV["HECKS_PLAYGROUND_DEBUG"]
        ensure
          if original
            HecksPlayground::CLI.define_singleton_method(:register_command, original)
          end
        end
      end

      # Load a new-style handle file. The file calls HecksPlayground::CLI.handle(:name) { |inv| ... }
      ...
      def load_handle_file(path)
        argv_instance = self
        original = HecksPlayground::CLI.method(:handle) rescue nil
        HecksPlayground::CLI.define_singleton_method(:handle) do |name, &block|
          next unless block
          argv_instance.handle(name.to_s) do |inv|
            LegacyContext.set_argv_ref(argv_instance)
            ctx = LegacyContext.new(inv, argv_instance)
        ...
        begin
          ...
          if original
            HecksPlayground::CLI.define_singleton_method(:handle, original)
          end
        end
      end
