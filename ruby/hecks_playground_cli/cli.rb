...
# commands via HecksPlayground::CLI.register_command with a group label.
#
begin
  require "thor"
rescue LoadError
  raise LoadError, "The hecks_playground CLI requires thor. Add gem 'thor' to your Gemfile."
end
require "fileutils"
require "hecks_playground/chapters/cli"
...
module HecksPlayground
  ...
  class CLI < Thor
    ...
    # hecks_playground_workshop → "Workshop", hecks_playground_cli/commands/build.rb → "Cli"
    def self.infer_group(path)
      if path =~ /hecks_playground_(\w+)/
        HecksPlayground::Utils.humanize(HecksPlayground::Utils.sanitize_constant($1))
      else
        "Commands"
      end
    end
    ...
    def print_grouped_help
      ...
      groups.each do |group_name, commands|
        ...
        commands.sort_by { |c| c[:name] }.each do |cmd|
          ...
          shell.say "  hecks_playground %-#{max_name}s  # %s" % [name, cmd[:description]]
        end
      end

      shell.say ""
      shell.say "Subcommands:", :bold
      shell.say "  hecks_playground gem build              # Build all component gems"
      shell.say "  hecks_playground gem install            # Install all component gems"
      shell.say "  hecks_playground docs update            # Update doc headers and markdown"
      shell.say ""
      shell.say "Run `hecks_playground help COMMAND` for details on a specific command."
    end

    # Subcommands
    # HecksPlayground::CLI::Gem
    #
    ...
    require "hecks_playground_cli/version_log_formatter"

    # Load commands from all hecks_playground modules
