HecksPlayground::CLI.handle(:build) do |inv|
  ...
  validator = HecksPlayground::Validator.new(domain)
  ...
  versioner = HecksPlayground::Versioner.new(".")

  latest_ver = HecksPlayground::DomainVersioning.latest_version(base_dir: ".")
  old_domain = latest_ver ? HecksPlayground::DomainVersioning.load_version(latest_ver, base_dir: ".") : nil
  bump_result = HecksPlayground::DomainVersioning::BreakingBumper.call(old_domain, domain, versioner)
  ...
  builder = HecksPlayground.target_registry[target.to_sym]
  unless builder
    say "Unknown build target: #{target}. Available: #{HecksPlayground.target_registry.keys.join(', ')}", :red
    next
  end
