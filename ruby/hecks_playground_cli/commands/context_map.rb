# HecksPlayground::CLI -- context_map command
#
...
#
#   hecks_playground context_map                    # text summary to stdout
#   hecks_playground context_map --mermaid          # Mermaid diagram to stdout
#   hecks_playground context_map --browser          # open diagram in browser
#   hecks_playground context_map --output map.md    # write Mermaid to file
#
HecksPlayground::CLI.handle(:context_map) do |inv|
  domains = load_context_map_domains
  next if domains.empty?

  generator = HecksPlayground::ContextMapGenerator.new(domains)
...
#
# @return [Array<HecksPlayground::BluebookModel::Structure::Domain>]
...
def open_context_map_in_browser(mermaid_markdown)
  ...
  html = <<~HTML
    ...
    <head>
      <meta charset="utf-8">
      <title>HecksPlayground Context Map</title>
    </head>
    <body>
      #{body}
      <script type="module">
  ...
  HTML
  require "tempfile"
  tmp = Tempfile.new(["hecks_playground_context_map", ".html"])
  tmp.write(html)
  tmp.close
