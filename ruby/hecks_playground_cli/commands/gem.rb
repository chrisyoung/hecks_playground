HecksPlayground::Chapters.load_aggregates(
  HecksPlayground::Cli::CliTools,
  base_dir: File.expand_path("..", __dir__)
)

module HecksPlayground
  class CLI < Thor
    # HecksPlayground::CLI::Gem
    #
    # Gem packaging commands -- delegates to GemBuilder to build and install
    # all HecksPlayground component gems from their subdirectories.
    #
    #   hecks_playground gem build
    #   hecks_playground gem install
    #
    class Gem < Thor
      desc "build", "Build all component gems and the meta-gem"
      # Builds every component gem from its own directory, then the meta-gem.
      #
      # @return [void]
      def build
        root = gem_root
        return unless root
        builder(root).build
      end

      desc "install", "Build and install all component gems locally"
      # Builds and installs every component gem, then the meta-gem.
      #
      # @return [void]
      def install
        root = gem_root
        return unless root
        builder(root).install
      end

      private

      def builder(root)
        GemBuilder.new(root, output: method(:say))
      end

      def gem_root
        root = Dir.pwd
        gemspec = ::File.join(root, "hecks_playground.gemspec")
        unless ::File.exist?(gemspec)
          say "hecks_playground.gemspec not found at #{root}", :red
          return nil
        end
        root
      end
    end
  end
end
