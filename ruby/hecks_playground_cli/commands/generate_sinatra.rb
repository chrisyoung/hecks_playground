HecksPlayground::CLI.handle(:generate_sinatra) do |inv|
  ...
  sinatra_app_rb = lambda do |d|
    ...
    <<~RUBY
      ...
      require_relative "config/hecks_playground"
  ...
  sinatra_hecks_playground_config = lambda do |d|
    <<~RUBY
  ...
  write_or_diff(File.join(app_name, "config/hecks_playground.rb"), sinatra_hecks_playground_config.call(domain))
  ...
  say "  config/hecks_playground.rb — domain configuration"
  say ""
  say "Next steps:"
  say "  cd #{app_name}"
  say "  bundle install"
