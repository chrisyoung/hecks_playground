HecksPlayground::CLI.handle(:info) do |inv|
  ...
  say_extensions = lambda do
    ...
    HecksPlayground::LoadExtensions::AUTO.each do |name|
      HecksPlayground::LoadExtensions.require_if_available(name)
      available << name.to_s if HecksPlayground.extension_registry.key?(name)
    end
    if available.any?
  ...
  say_services = lambda do
    ...
    files.each do |f|
      name = HecksPlayground::Utils.sanitize_constant(File.basename(f, ".rb"))
      say "  #{name}"
    end
    say ""
  end
  ...
  say_boundary_advisories = lambda do |domains|
    all_warnings = []
    domains.each do |d|
      validator = HecksPlayground::Validator.new(d)
      validator.valid?
      validator.warnings.each { |w| all_warnings << "#{d.name}: #{w}" }
    end
    if defined?(HecksPlayground::MultiDomain::Validator)
      HecksPlayground::MultiDomain::Validator.ambiguous_name_warnings(domains).each do |w|
        all_warnings << w
      end
    end
    if all_warnings.any?
