# HecksPlayground::CLI -- inspect command
#
...
#
#   hecks_playground inspect                          # full domain
#   hecks_playground inspect --aggregate Order        # single aggregate
#   hecks_playground inspect --domain path/to/domain  # explicit domain path
#   hecks_playground inspect --format json            # JSON output for tooling
#
...
HecksPlayground::CLI.handle(:inspect) do |inv|
  ...
  output = HecksPlayground::CLI::DomainInspector.new(domain).generate(aggregate: options[:aggregate])
  say output
end
