HecksPlayground::Chapters.load_aggregates(
  HecksPlayground::Cli::CliInternals,
  base_dir: File.expand_path("..", __dir__)
)

HecksPlayground::CLI.handle(:interview) do |inv|
  say "Welcome to HecksPlayground! Let's build your domain together.", :green
  say ""

  interviewer = HecksPlayground::Interviewer.new(
    ask: method(:ask),
    say: method(:say)
  )

  dsl_source = interviewer.run
  next say("Interview cancelled.", :yellow) unless dsl_source

  # Extract domain name from the generated DSL to name the file
  domain_name = dsl_source[/HecksPlayground\.domain "(\w+)"/, 1] || "MyDomain"
  write_or_diff("#{domain_name}Bluebook", dsl_source)
  write_or_diff("verbs.txt", "# Add custom action verbs here (one per line)\n# WordNet handles most English verbs automatically\n")

  say ""
  say "Next steps:", :green
  say "  hecks_playground validate          # check your domain"
  say "  hecks_playground build             # generate the domain gem"
  say "  hecks_playground console           # edit interactively"
end
