# HecksPlayground::CLI self_diff command
#
# Compares what generators would produce from a chapter's Domain IR
# against the actual code in the gem. Shows coverage gaps.
#
#   hecks_playground self_diff hecksagon                # domain mode (default)
#   hecks_playground self_diff hecksagon --framework    # framework skeleton mode
#
SELF_DIFF_CHAPTER_MAP ||= {
  "hecksagon"  => { gem: "hecksagon" },
  "bluebook"   => { gem: "bluebook" },
  "runtime"    => { gem: "hecksties" },
  "workshop"   => { gem: "hecks_playground_workshop" },
  "targets"    => { gem: "hecks_playground_targets" },
  "cli"        => { gem: "hecksties" },
  "extensions" => { gem: "hecksties" },
  "ai"         => { gem: "hecks_playground_ai" },
  "rails"      => { gem: "hecks_playground_on_rails" },
}.freeze

HecksPlayground::CLI.handle(:self_diff) do |inv|
  chapter_name = inv.args.first
  unless chapter_name
    say "Usage: hecks_playground self_diff <chapter> [--framework]", :red
  ...
  require "hecks_playground/chapters/#{key}"
  domain = HecksPlayground::Chapters.definition_from_bluebook(key)
  ...
  diff = HecksPlayground::Generators::Infrastructure::SelfHostDiff.new(
    domain, gem_root: gem_root, mode: mode
  )

  if mode == :framework
    gen = HecksPlayground::Generators::Infrastructure::FrameworkGemGenerator.new(
