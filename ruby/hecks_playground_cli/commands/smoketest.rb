# HecksPlayground::CLI smoketest command
#
# Context-aware: runs in the current directory.
# In a project dir (has hecks_playground/ but no examples/): runs that project's specs.
# In the hecks_playground root (has examples/): runs all example + framework specs.
#
#   cd examples/pizzas && hecks_playground smoketest   # tests pizzas
#   cd /path/to/hecks_playground && hecks_playground smoketest    # tests all examples + appeal
#
HecksPlayground::CLI.handle(:smoketest) do |inv|
  examples_dir = File.join(Dir.pwd, "examples")
  spec_dir = File.join(Dir.pwd, "spec")

  if File.directory?(examples_dir)
    # At the hecks_playground root — run everything
    passed = 0
    failed = 0
    errors = []
