# HecksPlayground::CLI::DomainInspector::AggregateFormatter::BehaviorFormatters
#
        ...
        module BehaviorFormatters
          ...
          def format_commands
            ...
            @agg.commands.each_with_index do |cmd, i|
              event = @agg.events[i]
              params = cmd.attributes.map { |a| "#{a.name}: #{HecksPlayground::Utils.type_label(a)}" }.join(", ")
              ...
              if cmd.call_body
                lines << "      body: #{HecksPlayground::Utils.block_source(cmd.call_body)}"
              end
            end
            lines << ""
          end
          ...
          def format_events
            ...
            @agg.events.compact.each do |ev|
              attrs = ev.attributes.map { |a| "#{a.name}: #{HecksPlayground::Utils.type_label(a)}" }.join(", ")
              lines << "    #{ev.name}(#{attrs})"
            end
            lines << ""
          end
          ...
          def format_queries
            ...
            @agg.queries.each do |q|
              body = HecksPlayground::Utils.block_source(q.block)
              lines << "    #{q.name}: #{body}"
            end
            lines << ""
          end
