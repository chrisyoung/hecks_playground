# HecksPlayground::CLI::DomainInspector::AggregateFormatter::RuleFormatters
#
        ...
        module RuleFormatters
          ...
          def format_invariants
            ...
            @agg.invariants.each do |inv|
              body = HecksPlayground::Utils.block_source(inv.block)
          ...
          def format_policy(pol)
            ...
            if pol.reactive?
              cond = pol.condition ? " when #{HecksPlayground::Utils.block_source(pol.condition)}" : ""
              "    #{pol.name}: #{pol.event_name} -> #{pol.trigger_command}#{async_note}#{cond}"
            else
              body = HecksPlayground::Utils.block_source(pol.block)
