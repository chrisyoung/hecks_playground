require "bundler"
  # HecksPlayground::GemBuilder
  #
  # Builds and installs all HecksPlayground component gems from their subdirectories,
  # then the meta-gem. Each component is built from its own directory so that
  # Dir globs in gemspecs resolve correctly.
  #
  #   builder = HecksPlayground::GemBuilder.new("/path/to/hecks_playground")
...
module HecksPlayground
  # HecksPlayground::GemBuilder
  #
  # Builds and installs all HecksPlayground component gems from their subdirectories, then the meta-gem.
  #
  class GemBuilder
    ...
    # @param root [String] path to the hecks_playground project root (must contain hecks_playground.gemspec)
    ...
    def build
      ...
      @output.call("Building hecks_playground meta-gem...", :green)
      Dir.chdir(root) { unbundled_system("gem build hecks_playground.gemspec") }
    end
    ...
    def install
      ...
      @output.call("Building hecks_playground meta-gem...", :green)
      Dir.chdir(root) do
        return build_failed("hecks_playground") unless unbundled_system("gem build hecks_playground.gemspec")
        gem_file = newest_gem("hecks_playground")
        return build_failed("hecks_playground") unless gem_file
        gem_files["hecks_playground"] = File.expand_path(gem_file)
      end
      ...
      meta = gem_files["hecks_playground"]
      return install_failed("hecks_playground") unless unbundled_system("gem install --local #{meta}")
      File.delete(meta)
    ...
    def bump_version!
      versioner = HecksPlayground::Versioner.new(root)
      new_version = versioner.next
      version_file = File.join(root, "hecksties", "lib", "hecks_playground", "version.rb")
