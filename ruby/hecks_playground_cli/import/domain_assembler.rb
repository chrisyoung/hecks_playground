module HecksPlayground
  module Import
    # HecksPlayground::Import::DomainAssembler
    #
    # Combines schema data and model data into a valid HecksPlayground DSL source
    # string. Maps Rails column types to HecksPlayground types, converts foreign
    ...
    class DomainAssembler
      ...
      def assemble
        lines = ["HecksPlayground.domain \"#{@domain_name}\" do"]
      ...
      def assemble_aggregate(table)
        ...
        if writable.any?
          ...
          writable.each do |col|
            hecks_playground_type = RAILS_TO_HECKS_PLAYGROUND[col[:type]] || "String"
            lines << "      attribute :#{col[:name]}, #{hecks_playground_type}"
          end
          lines << "    end"
        end
      ...
      def assemble_attribute(col, enums)
        if col[:type] == :reference
          ...
          hecks_playground_type = RAILS_TO_HECKS_PLAYGROUND[col[:type]] || "String"
          "    attribute :#{col[:name]}, #{hecks_playground_type}"
        end
      end
