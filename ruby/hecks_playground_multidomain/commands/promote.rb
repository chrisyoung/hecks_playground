HecksPlayground::CLI.register_command(:promote, "Extract an aggregate into its own domain",
  args: ["AGGREGATE"]
) do |aggregate_name|
  ...
  new_domain = HecksPlayground::BluebookModel::Structure::Domain.new(
    name: agg_name, aggregates: [agg], custom_verbs: []
  )
  new_file = "#{domain_snake_name(agg_name)}_domain.rb"
  File.write(new_file, HecksPlayground::DslSerializer.new(new_domain).serialize)
  ...
  updated = HecksPlayground::BluebookModel::Structure::Domain.new(
    name: domain.name, aggregates: remaining, custom_verbs: domain.custom_verbs
  )
  source_file = find_domain_file || "Bluebook"
  File.write(source_file, HecksPlayground::DslSerializer.new(updated).serialize)
