...
# dependency on hecks_playground. Orchestrates runtime templates, domain code,
...
class GemGenerator < HecksPlayground::Generator
  ...
  def generate_domain_rb(root)
    serializer = HecksPlayground::DslSerializer.new(@domain)
  ...
  def generate_gemspec(root, gem_name, mod)
    content = <<~RUBY
      Gem::Specification.new do |s|
        s.name        = "#{gem_name}"
        s.version     = "#{@version}"
        s.authors     = ["Generated by HecksPlayground"]
  ...
  def generate_server(root, gem_name, mod)
    ...
    explorer_dir = File.expand_path("../../../../../hecksties/lib/hecks_playground/extensions/web_explorer", __dir__)
    explorer_dir = File.expand_path("../../../../hecks_playground_runtime/lib/hecks_playground/extensions/web_explorer", __dir__) unless Dir.exist?(explorer_dir)
    renderer_src = File.read(File.join(explorer_dir, "renderer.rb"))
    renderer_out = renderer_src
      .gsub("module HecksPlayground\n  module WebExplorer", "module #{mod}\n  module Server")
      .gsub("HecksPlayground::WebExplorer", "#{mod}::Server")
