...
#
class UIGenerator < HecksPlayground::Generator
  include FormRoutes
  ...
  def self_ref?(cmd, agg_snake)
    HecksPlayground::Conventions::CommandContract.find_self_ref(cmd.attributes, agg_snake) != nil
  end

  def find_self_ref_attr(cmd, agg)
    HecksPlayground::Conventions::CommandContract.find_self_ref(cmd.attributes, agg.name)
  end
