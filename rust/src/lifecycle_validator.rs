...
/// Per i563/i568 — a lifecycle-tracked attribute wrapped as a VO
/// MUST carry an explicit `default:` clause. Without it the
/// dispatcher reads the wrapped VO back as `"[0 items]"` and the
/// lifecycle's `from:` clauses fail at the first transition. The
/// runtime gives primitive-typed lifecycle attrs (String/Integer)
/// a sensible empty value the lifecycle's `from:` can still match,
/// so this check only fires for the VO-wrap form.
fn check_lifecycle_attr_missing_default(agg: &Aggregate, out: &mut Vec<Finding>) {
