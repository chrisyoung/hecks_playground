...
fn parallel_across_aggregates_no_block() {
    ...
    // The parallel drain spawns one OS thread per mailbox. Each thread
    // hits the barrier ; the test passes only when both reach the wait
    // simultaneously — proving cross-actor parallelism. If actor A were
    // blocked behind actor B the barrier would never lift and the test
