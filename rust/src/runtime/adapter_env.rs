//! runtime::adapter_env — map a `.world` adapter block onto the handler child env.
//!
...
//!
//! Relocated from `run_host::config` (i750 out-of-process-adapter pump) so the
//! runtime pump (`reaction::pump_outbound_events`) — the new in-runtime drain of
//! the OutboundEvent outbox — can resolve a handler's env WITHOUT depending on
//! the soon-to-retire `run_host` standalone-host module. `run_host::config`
//! re-exports this for the duration of its (transitional) life.
