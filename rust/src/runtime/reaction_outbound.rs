//! reaction_outbound — pump_outbound_events, the OUT-OF-PROCESS
//! `OutboundEvent` outbox drain (i750) : claim-before-spawn, resolve
//! handler + `.world` config into the child env, detach-spawn the handler
//! with the re-entry env contract. The verdict + MarkDelivered ack are the
//! handler's job. Host-only (cfg'd out of wasm). Inherent `impl Runtime`
//! method in a child module, same contract as reaction.rs.
//!
//! Cask extracted VERBATIM from runtime/reaction.rs (cask-runtime).
//!
//! [antibody-exempt: rust/src/runtime/reaction_outbound.rs — hand-written
