//! reaction_outbox — pump_outbox, the reaction-outbox drain : deliver every
//! Active run's steps as their OWN transactions (each step a separate
//! dispatch on this later tick), then mark the run Completed. Idempotent at
//! the run grain. The pump is the ONLY thing that turns an outbox entry
//! into a sibling mutation — across a transaction boundary, never inline.
//! Inherent `impl Runtime` method in a child module, same contract as
//! reaction.rs.
//!
//! Cask extracted VERBATIM from runtime/reaction.rs (cask-runtime).
//!
//! [antibody-exempt: rust/src/runtime/reaction_outbox.rs — hand-written
