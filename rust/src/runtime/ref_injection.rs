//! ref_injection — reference RESOLUTION for cascades and events :
//! inject_refs (for each reference on a triggered command, inject the id the
//! cascade needs — lazy string-match + IdentifierService derivation, the
//! i611/i614 cross-aggregate contract) and event_refs (the belongs_to /
//! reference_to leaves an event carries for the causation tree).
//!
//! Cask extracted VERBATIM from runtime/mod.rs (shrink-mod phase B, wave 3).
//!
//! [antibody-exempt: rust/src/runtime/ref_injection.rs — kernel-floor
//!  reference resolution, relocated verbatim from mod.rs blanket.]
