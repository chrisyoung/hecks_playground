  ...
  module Vocabulary
    TABLES = {
      ...
      "DomainRefusal" => [
        ...
        {"name"=>"EntityDispatchRefused"}.freeze,
      ...
      "RefusalTemplate" => [
        ...
        {"refusal"=>"EntityDispatchRefused", "site"=>"direct_dispatch", "template"=>"{entity} is {aggregate}'s own nested entity — its commands run only through {aggregate}'s own reactions, never dispatched directly"}.freeze,
        ...
        {"refusal"=>"TypeMismatch", "site"=>"arithmetic_field_mismatch", "template"=>"{op} of {target} refused — its own {field} disagrees with the source's, and this op only ever updates the one shared numeric field, silently keeping every other one exactly as {target} already had it"}.freeze,
