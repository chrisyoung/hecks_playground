...
module RustProjection
  ...
  module DomainGenerator
    ...
    # i754 -- when the lifecycle field is ALSO one of NODE's own declared
    # attributes (the normal case -- see `Projector.lifecycle_field_
    # declared?`), `emit_to_json_flat`/`emit_from_json_state`'s own
    # attribute-driven loop already serializes it correctly, VO-aware
    # (a closed-set field routes through its own `to_json`/`from_json`,
    # generated by `emit_closed_set_codec`). Adding this extra field on
    # top used to duplicate the JSON key on the way out and duplicate the
    # struct-literal field on the way in (`from_json`'s own generated
    # `Self { ..., status: ..., status: ... }`, a real E0062). Only a
    # field-less lifecycle (not reachable via the current grammar, but not
    # ruled out defensively) still needs this fallback.
    def lifecycle_extra_field(node)
      return [] unless node[:lifecycle]
      return [] if Projector.lifecycle_field_declared?(node)
    ...
    def call(ir, source_label, mod_dir, mod_name)
      ...
      ir[:aggregates].each do |aggregate|
        ...
        File.open(path, "w") do |f|
          ...
          f.puts "#![allow(dead_code, unused_variables, unused_imports)]"
